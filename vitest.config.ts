...
export default defineConfig({
  test: {
     ...
     * jsdom de este repo cruzan las fronteras perezosas de `notas/perezosos.tsx`
     * (BlockNote, React Flow), y **ese costo se paga UNA sola vez**: lo carga el
     * primer test que toque montarlas, y los demás del archivo entran gratis.
     *
     * Medido el 21-ago-2026 corriendo los dos archivos solos: el primer test de
     * `Libreta.dibujo` tarda 9,1 s, el primero de `Libreta.dividirPantalla`
     * 9,0 s, y el primero que monta React Flow 5,2 s — mientras que los otros 46
     * van de 70 a 800 ms. O sea que el número no estaba midiendo el test: medía
     * a cuál le tocó estrenar el módulo. Con 5 s, N2 quedaba rojo en 10 tests
     * que pasan perfecto de a uno.
     *
     * Es el mismo falso rojo que `esperarA` ya documenta para el tope de TURNOS
     * —«un acoplamiento oculto al tamaño del bundle»—, reaparecido con forma de
     * milisegundos. Lo que ataja un cuelgue de verdad no es este número sino
     * `TOPE_ANTI_CUELGUE` (`src/pruebas/dom.tsx`), que cuenta turnos y no mira el
     * reloj: un test trabado sigue muriendo en segundos, no en treinta.
     */
    testTimeout: 30_000,
    hookTimeout: 30_000,
