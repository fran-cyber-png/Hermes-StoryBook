 ...
 */
// Se exporta desde el 24-ago-2026, y ya tiene DOS consumidores que llegaron el
// mismo día por el mismo motivo: `PasarConversacion` la necesita para decidir si
// el chip dice «Tú», y `dominio/tenencia.ts` (ADR 0083) para decidir si este chat
// lo estás atendiendo tú o alguien más. Reimplementarla en cualquiera de los dos
...
export function nombreCorto(vendedoraId: string): string {
  /**
   * ⚠️ **El namespace se saca ANTES de cortar**, y no es cosmética menor: `:` no
   * está entre los separadores de acá, así que `centurion:diego.tapia` salía como
   * **«Centurion:diego»** — el rótulo nombra al SISTEMA de identidad en vez de a
   * la persona, y en el módulo de campaña TODAS las identidades tienen ese
   * prefijo (`auth/centurion.ts:vendedoraIdDeCenturion`), así que ahí no se
   * salvaba ninguna. El prefijo sigue existiendo en el id y en el `title`: lo
   * que cambia es sólo cómo se lee.
   */
  const sinNamespace = vendedoraId.trim().replace(/^[a-z]+:/i, '');
  const base = sinNamespace.split(/[.\-_@\s]/)[0] ?? '';
