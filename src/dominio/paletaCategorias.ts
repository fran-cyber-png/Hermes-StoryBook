...
/**
 * QUÉ COLOR LE TOCA AL AVATAR DE UN CONTACTO (rediseño 28-ago-2026, pedido del
 * dueño: círculos de colores variados, no todos del mismo gris). Reusa la MISMA
 * paleta de ocho colores que las categorías (`COLORES`) — inventar una segunda
 * paleta de colores acá sería el mismo error que el docblock de `PildoraCanal`
 * ya dice haber arreglado una vez con el canal.
 *
 * El hash es determinístico por la semilla: el MISMO contacto cae SIEMPRE en el
 * mismo color, en cualquier fila donde aparezca y en cualquier recarga — no es
 * un color al azar en cada render.
 */
export function colorDeAvatar(semilla: string): ColorCategoria {
  let hash = 0;
