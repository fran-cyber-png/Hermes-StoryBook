...
/**
 * ══ EL EMBUDO DEL DASHBOARD — EL CANDADO DEL ISSUE #329 ═══════════════════
 *
 * El defecto: `VistaDashboard.tsx` sumaba y dibujaba iterando `ETAPAS`, que
 * excluye `sin_respuesta` a propósito. El radar mostraba 1.397 cuando el
 * seam devolvía 3.973 — el 65 % del embudo, la etapa más grande, nunca se
 * sumaba ni se dibujaba. El fix: el total y el orden de dibujo salen de las
 * CLAVES QUE EL SERVER MANDÓ, nunca de una lista fija del front.
 */
describe('totalDelEmbudo — la suma de TODAS las claves, no de una lista del front', () => {
  it('🔴 el caso medido del issue #329: 3.973, no 1.397', () => {
    const embudo = {
      sin_respuesta: 2576,
      interesado: 377,
      contactado: 217,
    ...
    expect(totalDelEmbudo(embudo)).toBe(3973);
    // El bug reproducido con la implementación vieja, para que quede escrito
    // cuál era el error exacto: sumar solo lo que ETAPAS enumera.
    const totalViejo = ETAPAS.reduce((n, e) => n + (embudo[e as keyof typeof embudo] ?? 0), 0);
    expect(totalViejo).toBe(1397);
    expect(totalViejo).not.toBe(totalDelEmbudo(embudo));
  });

  it('degrada a 0 sin datos, nunca tira', () => {
    expect(totalDelEmbudo(null)).toBe(0);
    expect(totalDelEmbudo(undefined)).toBe(0);
    expect(totalDelEmbudo({})).toBe(0);
  });

  it('suma una etapa futura que ni `ETAPAS` ni `SIN_RESPUESTA` conocen', () => {
    // El server puede empezar a devolver un peldaño nuevo antes de que el
    // front lo conozca (N4/N5 se despliegan por separado) — el total no se
    // puede quedar corto por eso.
    expect(totalDelEmbudo({ interesado: 5, peldano_del_futuro: 3 })).toBe(8);
  });
});

describe('ordenDelEmbudo — ninguna clave se pierde', () => {
  it('antepone sin_respuesta y respeta el orden de siempre para el resto', () => {
    const claves = ['cierre', 'sin_respuesta', 'cotizado', 'interesado', 'contactado', 'perdido'];
    expect(ordenDelEmbudo(claves)).toEqual([
      'sin_respuesta',
      'interesado',
      'contactado',
      'cotizado',
  ...
  it('una clave que faltó en la respuesta simplemente no aparece — no inventa ceros', () => {
    expect(ordenDelEmbudo(['interesado', 'cierre'])).toEqual(['interesado', 'cierre']);
  });

  it('🔴 degrada, no tumba: una clave desconocida se agrega al final, nunca se pierde', () => {
    expect(ordenDelEmbudo(['interesado', 'peldano_del_futuro'])).toEqual([
      'interesado',
      'peldano_del_futuro',
    ]);
  });
...
describe('colorSegmento — por CLAVE, nunca por posición', () => {
  it('🔴 agregar sin_respuesta como primer segmento no corre el color de los demás', () => {
    // Antes: `['bg-navy/40','bg-navy/60','bg-navy'][i]` indexaba por POSICIÓN
    // en `ETAPAS`. Con sin_respuesta antepuesto, interesado pasaba de i=0 a
    // i=1 y se pintaba con el color de contactado. Por clave, no importa
    // dónde caiga cada una en la lista.
    const antes = colorSegmento('interesado');
    const conSinRespuestaAdelante = colorSegmento('interesado');
    expect(antes).toBe(conSinRespuestaAdelante);
    expect(colorSegmento('sin_respuesta')).not.toBe(colorSegmento('interesado'));
    expect(colorSegmento('sin_respuesta')).not.toBe(colorSegmento('contactado'));
  });

  it('degrada a un color válido con una etapa desconocida, nunca tira', () => {
    expect(() => colorSegmento('peldano_del_futuro')).not.toThrow();
    expect(colorSegmento('peldano_del_futuro')).toMatch(/^bg-/);
  });
});
