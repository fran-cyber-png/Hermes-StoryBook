...
/**
 * La red de 5 minutos que comparten TODOS los polls de la raíz con el stream
 * vivo — la cola y, desde el PR 1 de `docs/plan-borrar-el-polling.md`, la
 * sesión de WhatsApp, la agenda, el bot y la auto-respuesta.
 */
export const RED_CON_STREAM_MS = 300_000;
...
export function intervaloDeCola(vivo: boolean, azar: number): number {
  if (vivo) return RED_CON_STREAM_MS;
  return COLA_SIN_STREAM_BASE_MS + azar * COLA_SIN_STREAM_JITTER_MS;
}

/**
 * LA REGLA GENERALIZADA — la misma que `intervaloDeCola`, para cualquier poll
 * de la raíz que no tenga su propio jitter (docs/plan-borrar-el-polling.md §6,
 * PR 1).
 *
 * Con el stream vivo, una red de 5 minutos: el push ya invalida esa clave al
 * instante (`lib/datos/tiempoReal.ts`) o, si no la invalida, el cambio no es
 * urgente (la agenda lee el reloj del cliente; auto-respuesta/bot ganan un
 * evento propio en el PR 3). Sin stream, `sinStreamMs` es el ritmo de
 * SIEMPRE — el que cada consumidor ya tenía antes de este frente.
 *
 * `intervaloDeCola` no se reescribe en términos de ésta: lleva jitter propio
 * (`COLA_SIN_STREAM_JITTER_MS`) que los consumidores de acá no necesitan —
 * no son la consulta más cara del sistema, y sincronizarlos entre pestañas no
 * cuesta lo que cuesta sincronizar la cola.
 */
export function intervaloConStream(vivo: boolean, sinStreamMs: number): number {
  return vivo ? RED_CON_STREAM_MS : sinStreamMs;
}
