...
describe('intervaloDeCola', () => {
  it('con el stream vivo, la cola es una red muy espaciada', () => {
    expect(intervaloDeCola(true, 0)).toBe(RED_CON_STREAM_MS);
    expect(intervaloDeCola(true, 0.99)).toBe(RED_CON_STREAM_MS);
  });
...
describe('intervaloConStream', () => {
  it('con el stream vivo, siempre la red de 5 minutos — sin importar lo que pida el consumidor', () => {
    expect(intervaloConStream(true, 30_000)).toBe(RED_CON_STREAM_MS);
    expect(intervaloConStream(true, 60_000)).toBe(RED_CON_STREAM_MS);
  });

  it('sin stream, el ritmo de siempre de CADA consumidor — no un número clavado acá', () => {
    expect(intervaloConStream(false, 30_000)).toBe(30_000);
    expect(intervaloConStream(false, 60_000)).toBe(60_000);
  });
});
