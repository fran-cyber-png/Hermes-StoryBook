...
import type { CorreoCompleto, CorreoEnLista, EstadoDeCorreos, HiloDeBandeja, Remitente } from './tipos';
...
const CASO = {
  ...
  leer: P.has('leer'),
  /**
   * Con qué carpeta abre la bandeja. `?riel=recibidos` es el caso que hay que
   * poder mirar: la carpeta cableada y VACÍA, con el cartel que dice por qué.
   */
  riel: (P.get('riel') ?? undefined) as Riel | undefined,
  /** `?redactar=1` — el composer abierto sobre la bandeja. */
  redactar: P.has('redactar'),
  /**
   * `?tildado=1` — con todo seleccionado, para fotografiar la barra de acciones
   * masivas. Sin esto no hay forma de capturarla: se dibuja sólo con selección y
   * el navegador headless no hace clic.
   */
  tildado: P.has('tildado'),
};
...
const CORREOS: CorreoEnLista[] = [
  /**
   * ⚠️ **Son las filas de PRODUCCIÓN, no un caso ideal** (regla #9 de la casa: una
   * galería que no sirve los valores reales no es evidencia). Las tres primeras son
   * de agosto y las dos de arriba se agregaron el 24-ago-2026 leyéndolas de la
   * pantalla viva: `desde` en hueco —salieron antes de la 0029— y asuntos de
   * prueba, que es exactamente lo que hay. Un fixture con asuntos redactados
   * escondería que el avance del cuerpo llega vacío en las filas viejas.
   */
  {
    id: 5,
    vendedoraId: 'alex',
    para: 'ecgoberna@gmail.com',
    asunto: 'PRUEBA',
    estado: 'enviado',
    motivo: null,
    desde: null,
...
/** Las etiquetas del riel. Dos, con colores de la paleta compartida. */
const ETIQUETAS = [
  { id: 1, nombre: 'Cotizaciones', color: 'azul', creadoPor: VENDEDORA },
  { id: 2, nombre: 'Postventa', color: 'verde', creadoPor: VENDEDORA },
];

/**
 * Un enviado, en la forma que sirve `GET /api/correos/bandeja`.
 *
 * ⚠️ El `avance` sale de `CUERPOS`, que es el mismo mapa que usa el detalle: en
 * producción ese texto lo recorta Postgres con `left(cuerpo, 140)`, así que acá se
 * recorta igual — una galería que muestra el avance entero fotografiaría un
 * renglón que no existe.
 */
/**
 * A dónde se va una fila con cada acción. `undefined` = no se mueve de carpeta,
 * sólo le cambia un campo.
 *
 * ⚠️ Es un mapa de la GALERÍA, no una regla: en producción quién termina dónde lo
 * decide el `WHERE` de `carpetas.ts`, y acá sólo se simula el movimiento visible.
 * Posponer no está: sin el reloj del server no se puede simular sin mentir.
 */
...
/**
 * La bandeja de la galería, por carpeta. Es lo ÚNICO mutable de este archivo.
 * `ESTANTE` y no `CORREOS` para que se lea que es otra cosa: `CORREOS` es la
 * auditoría inmutable de las cinco filas de producción, esto es dónde está cada
 * una ahora mismo en esta pestaña.
 */
const ESTANTE: Record<string, HiloDeBandeja[]> = {
  enviados: CORREOS.map(comoHiloDeGaleria),
  /**
   * ⚠️ **Archivados arranca con una fila y no vacía, y hace falta que así sea.**
   * El botón de desarchivar sólo se dibuja con algo tildado, y el navegador
   * headless no hace clic: con la carpeta vacía la captura mostraría el vacío
   * honesto y no la acción que hay que poder mirar. Es la fila 4 de producción,
   * la misma de Enviados, puesta donde caería si alguien la archivara — no un
   * correo inventado.
   */
  archivados: CORREOS.filter((c) => c.id === 4).map((c) => ({
    ...comoHiloDeGaleria(c),
    carpeta: 'archivado',
  })),
  /**
   * 🔴 **Éste SÍ es inventado, y se dice.** En producción hay **cero** borradores
   * —la tabla los soporta desde la 0049 y hasta el 25-ago-2026 no existía una sola
   * línea que guardara uno—, así que no hay valor real que servir. Lo que se
   * fotografía es la forma del primero que va a existir: sin `desde` (todavía no
   * salió por ningún buzón) y con el avance a medio escribir, que es lo que un
   * borrador es.
   */
  borradores: [
    {
      ...comoHiloDeGaleria({
        id: 6,
        vendedoraId: 'luz',
        para: 'ana.quispe@gmail.com',
        asunto: 'Diploma en Gestión Pública — fechas y precio',
        estado: 'enviado',
        motivo: null,
        desde: null,
        responderA: null,
        creadoAt: '2026-08-25T09:12:00.000Z',
      }),
      estado: 'borrador',
      avance: 'Ana, gracias por escribirnos. El diploma arranca el 8 de septiembre y',
    },
  ],
};

/** El cuerpo del POST que se está atendiendo. Lo deja `window.fetch`, más abajo. */
let ultimoCuerpo: Record<string, unknown> = {};
const cuerpoDelPost = () => ultimoCuerpo;

/**
 * ⚠️ El retorno se ANOTA como `HiloDeBandeja` en vez de inferirse. Inferido, el
 * `estado` quedaba en `'enviado' | 'fallido'` —lo que sirve la lista de enviados—
 * y el estante no podía sostener un borrador, que es un estado perfectamente
 * válido de la bandeja. El tipo del contrato es el que manda acá.
 */
function comoHiloDeGaleria(c: CorreoEnLista): HiloDeBandeja {
  return {
    hilo: `correo:${c.id}`,
    id: c.id,
    vendedoraId: c.vendedoraId,
    para: c.para,
    // ⚠️ `?? null` y no `c.desde` a secas: en `CorreoEnLista` el campo es opcional
    // —las filas anteriores a la 0029 no lo tienen— y en el hilo es `string | null`.
    // El hueco es el dato, así que se conserva como hueco explícito.
    desde: c.desde ?? null,
    asunto: c.asunto,
    clave: null,
    estado: c.estado,
    ...
    destacado: c.id === 5,
    importante: false,
    avance: (CUERPOS[c.id] ?? '').slice(0, 140),
    etiquetas: c.id === 5 ? [ETIQUETAS[0]] : [],
  };
}
...
function responder(url: string, metodo: string): Respuesta {
  ...
  /**
   * ⚠️ **La bandeja se DERIVA del mismo `CORREOS`, no se escribe aparte.** Dos
   * fixtures del mismo dato es #37 en su versión de galería: se separan, y la foto
   * deja de mostrar lo que la pantalla muestra con datos de verdad.
   */
  if (url.includes('/api/correos/bandeja')) {
    const riel = /riel=([a-z]+)/.exec(url)?.[1] ?? 'recibidos';
    // Sólo Enviados y Archivados tienen filas: Hermes todavía no recibe correo, y
    // la galería tiene que servir ESO —el vacío honesto— y no una bandeja
    // inventada. Archivados existe porque es a donde van las que se archivan acá.
    const hilos = ESTANTE[riel] ?? [];
    return ok({ riel, total: hilos.length, pagina: 1, hilos });
  }

  /**
   * LAS ACCIONES, que acá SÍ mueven las filas.
   *
   * ⚠️ **Esto es lo único de la galería que muta su estado, y es a propósito.**
   * El resto contesta siempre lo mismo porque una galería que cambia deja de
   * servir el mismo caso en la segunda captura (por eso el alta de un remitente
   * contesta `ok` y no toca nada). Pero un preview donde tocás «Archivar» y no
   * pasa NADA se lee como roto, que es peor que no tener el botón. El estado vive
   * en memoria: recargar vuelve al caso de siempre, así que las capturas de
   * evidencia —que cargan la página de cero— no se ven afectadas.
   *
   * 🔴 **Y NO reimplementa el predicado de las carpetas.** Eso vive una sola vez,
   * en `server/src/correos/carpetas.ts`, y copiarlo acá sería la trampa de #37
   * justo en el archivo que se usa como evidencia: la foto mostraría un criterio
   * y producción otro. Lo que hace es lo que se puede afirmar sin el predicado:
   * mover la fila de estante, o cambiarle un campo donde está.
   */
  if (url.includes('/api/correos/acciones')) {
    const pedido = cuerpoDelPost();
    const ids = new Set((pedido.ids as number[]) ?? []);
    const accion = String(pedido.accion ?? '');

    const destino = DESTINO[accion];
    let tocadas = 0;

    for (const [riel, hilos] of Object.entries(ESTANTE)) {
      for (const h of [...hilos]) {
        if (!ids.has(h.id)) continue;
        tocadas++;
        if (destino !== undefined) {
          ESTANTE[riel] = ESTANTE[riel]!.filter((x) => x.id !== h.id);
          (ESTANTE[destino] ??= []).unshift(h);
        } else if (accion === 'destacar') h.destacado = true;
        else if (accion === 'no-destacar') h.destacado = false;
        else if (accion === 'leer') h.sinLeer = false;
        else if (accion === 'no-leer') h.sinLeer = true;
      }
    }
    return ok({ ok: true, tocadas, pedidos: ids.size });
  }

  if (url.includes('/api/correos/etiquetas')) return ok({ etiquetas: ETIQUETAS });
...
window.fetch = (async (entrada: RequestInfo | URL, init?: RequestInit) => {
  ...
  // El cuerpo del POST se deja acá para que `responder` lo pueda leer sin cambiar
  // su firma —la usan ocho ramas y siete no lo necesitan—. Un JSON que no parsea
  // es un objeto vacío: la rama que lo lea va a contestar «no hay ids», que es la
  // respuesta correcta a un pedido ilegible.
  try {
    ultimoCuerpo = init?.body ? (JSON.parse(String(init.body)) as Record<string, unknown>) : {};
  } catch {
    ultimoCuerpo = {};
  }
...
async function conducir(): Promise<void> {
  ...
  escribir(
    ...
    'Hola, te paso la información del Diploma y las formas de pago. Cualquier duda me escribes por acá.',
  );
  await esperar(150);
...
/**
 * 🔴 **LA GALERÍA FIJA EL TEMA, Y ESO ES UNA CORRECCIÓN DE LA EVIDENCIA.**
 *
 * Sin esto el `data-theme` no se estampa nunca, así que la página queda a merced
 * de `prefers-color-scheme` — y un Chrome headless, además, le aplica su **auto
 * dark** encima: oscurece una página CLARA por heurística. El resultado es una
 * captura que se ve oscura y **no es el tema oscuro de Hermes**: los tokens que
 * se leen son los claros (`--muted: #F5F7FB`), y un `bg-muted/40` sale gris
 * medio en vez del slate que usa la app. Una foto así no es evidencia de nada:
 * muestra colores que ninguna vendedora va a ver.
 *
 * `?claro=1` fotografía el otro tema. El default es oscuro porque es como corre
 * la app de escritorio.
 */
document.documentElement.dataset.theme = P.has('claro') ? 'light' : 'dark';
  ...
  <StrictMode>
    <QueryClientProvider client={qc}>
      <div className="flex h-screen flex-col bg-background">
        ...
        */}
        <VistaCorreos
          rielInicial={CASO.riel}
          tildarTodoAlAbrir={CASO.tildado}
          abrirComposer={CASO.redactar}
        />
      </div>
    </QueryClientProvider>
  </StrictMode>,
