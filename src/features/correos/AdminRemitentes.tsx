 ...
 * despliegue, y «no eres supervisora» manda a pedírselo a alguien. Colapsarlas en un
 * «no tienes permiso» haría que, durante la ventana N4↔N5 o con un blip de red, una
...
function SinPermiso({
  ...
  return (
    <div className="flex items-start gap-2 rounded-xl bg-muted/60 p-3.5 text-xs leading-relaxed text-foreground">
      ...
      {loDijoElServer ? (
        <p>
          Los remitentes los administra quien está a cargo del equipo. Pídele que dé de alta el buzón que
          necesitas — tú puedes seguir mandando correos con normalidad.
        </p>
      ) : (
        <p>
        ...
        <label className="flex flex-col gap-1">
          ...
          {!p.direccionEditable && (
            <span className="text-[10px] leading-snug text-muted-foreground">
              ...
              otro, da de baja éste y crea uno nuevo.
            </span>
          )}
        </label>
...
function VistaPreviaDelSobre({
  ...
  if (!FORMA_DE_CORREO.test(direccion) || !dominioAceptado(direccion, dominios)) {
    return (
      <p className="rounded-lg border border-dashed border-border px-2.5 py-2 text-[10px] leading-snug text-muted-foreground">
        Escribe una dirección de un dominio verificado y acá se dibuja, tal cual, el sobre que va a recibir el
        lead.
      </p>
    );
  }
  ...
  return (
    <div className="rounded-lg bg-muted/60 px-2.5 py-2">
      <p className={kicker}>Así sale cuando escribes tú</p>
