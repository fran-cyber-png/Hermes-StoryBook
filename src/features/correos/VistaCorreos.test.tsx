...
async function abrir(
  ...
  /**
   * 🔴 **EL COMPOSER YA NO ES LA PANTALLA: HAY QUE ABRIRLO.** Correos pasó de ser
   * un formulario con una lista debajo a ser una BANDEJA con el composer en un
   * panel encima («Redactar»). Los 20 casos de este archivo interrogan al composer
   * —el sobre, el ritmo, el puente, las dos puertas de enviar— y ninguno de ellos
   * cambió de significado: lo único que cambió es que antes estaba montado al
   * entrar y ahora se abre.
   *
   * ⚠️ **Se abre acá, en el helper compartido, y no caso por caso**: con el clic
   * repetido veinte veces, el día que el composer se abra de otra manera hay que
   * tocar veinte lugares y el que se olvide falla con «no encontré el textarea»,
   * que no dice nada de la causa.
   */
  const redactar = [...montado.contenedor.querySelectorAll('button')].find((b) =>
    (b.textContent ?? '').includes('Redactar'),
  );
  if (!redactar) throw new Error('no está el botón «Redactar»: ¿cambió la barra superior de Correos?');
  tocar(redactar);
...
describe('VistaCorreos — la pantalla MUESTRA el sobre en vez de prometerlo', () => {
  ...
  it('dice a dónde vuelve la respuesta cuando el `vendedora_id` ES un buzón', async () => {
    const { contenedor } = await abrir(estadoQueContesta(), 'ventas11@grupogoberna.com');
    expect(contenedor.textContent).toContain('Las respuestas te llegan a ti');
  ...
  it('con el composer vacío el botón está apagado y dice POR QUÉ', async () => {
    ...
    expect(contenedor.textContent).toContain('Falta a quién se lo mandas.');
  });
