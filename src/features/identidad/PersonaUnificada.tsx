...
import { BadgeCheck, Link2, Loader2, Unlink, Users } from 'lucide-react';
 ...
 *
 * «A veces hablas con la misma persona pero te escribe de otro número.» Acá la vendedora lo
  ...
  return (
    <div className="mt-5">
      <div className={encabezadoSeccion}>
        <Users size={14} className="text-muted-foreground" /> La misma persona
      </div>
      ...
      {isPending ? (
        ...
        <p className="mt-1.5 text-[11px] leading-relaxed text-muted-foreground">
          Si esta persona también te escribe desde otro número o desde otra red, une las dos fichas.
        </p>
      ) : (
      ...
      <button
        ...
        className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-full border border-dashed border-border py-2 text-xs font-bold text-primary transition-colors duration-200 ease-house hover:border-primary hover:bg-primary/5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"
      >
        <Link2 size={13} /> Unir Fichas
      </button>

      {deshacer.isError && (
        <p className="mt-1.5 text-[11px] text-destructive">No se pudo deshacer — prueba de nuevo.</p>
      )}
