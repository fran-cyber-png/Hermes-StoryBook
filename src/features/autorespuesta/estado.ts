...
export const QUE_HACE: Record<ModoAutoRespuesta, string> = {
  ...
  supervisada:
    'La cola prepara las respuestas fuera de horario y esperan tu OK. Las revisas en el chat, con la conversación a la vista. No sale nada sin que lo apruebes.',
...
export function verAutoRespuesta(
  ...
  if (modo === 'apagada') {
    ...
    return {
      ...
      detalle: frenada
        ? `${datos.interruptor?.motivo}. Se frenó sola y no vuelve sola: mira el motivo antes de prenderla.`
        : 'nadie recibe respuestas automáticas. Antes de prenderla, corre el simulacro (`npm run auto:simulacro`).',
      puedeCambiar: true,
      ...vacia,
    };
  }
 ...
 * automática, cuántas van a salir y cuándo sale la próxima (es lo que tienes que
