 ...
 *
 * El estado puede cambiar SIN que nadie toque el chip —el freno automático lo
 * apaga solo ante un ban o un error de envío, y las preparadas caducan solas—
 * y una pantalla que siga diciendo «encendida» o «12 esperando» en ese caso
 * estaría mintiendo. Por eso se refresca solo: con el stream vivo, cada 5
 * minutos (`intervaloConStream`, `docs/plan-borrar-el-polling.md` §6 PR 1) —
 * el `PUT` propio relee en el acto vía `onSettled`, así que la acción nunca
 * espera al poll; sin stream, cada 30 s, el ritmo de siempre.
 *
