...
/**
 * LA FRONTERA PEREZOSA DE LA LIBRETA — entrar a la vista no es abrir una
 * página.
 *
 * ══ 🔴 LO QUE COSTABA TENERLA ADENTRO ═══════════════════════════════════════
 *
 * La vista entera venía en UN chunk perezoso, así que entrar a la Libreta
 * —aunque fuera a buscar una nota vieja y volverse— bajaba y parseaba
 * BlockNote. Medido con `vite build` sobre el árbol de este PR:
 *
 *     entrar a la Libreta        377,8 KB gzip  →   19,7 KB   (−95 %)
 *     abrir una página de texto        (venía)  →  288,6 KB + 35,8 de CSS
 *
 * Y en la máquina de la vendedora esto no está precargado: `index.html` no
 * lleva `modulepreload` de este chunk, así que se descarga al entrar, en su
 * red.
 *
 * ══ POR QUÉ VIVE EN ESTE ARCHIVO Y NO EN CADA LLAMADOR ══════════════════════
 *
 * `Libreta.tsx` y `PantallaDividida.tsx` montan el MISMO componente. Con un
 * `lazy()` en cada uno serían dos fronteras para un módulo: cada una con su
 * `Suspense`, su esqueleto y su forma de fallar — y un arreglo en una no
 * llegaría a la otra (#37). Acá hay una sola, y las dos pantallas la
 * comparten.
 *
 ...
 */
export function ColumnaDeEscritura({
  children,
  dividida,
  registrarHojaA4,
}: {
  ...
  dividida?: boolean;
  /**
   * Le presta al padre el nodo DOM de `.hoja-a4` (o `null` al desmontarse) —
   * lo necesita `CapaDeAnotaciones` para portar ahí su canvas, que si no
   * queda afuera de la única caja que de verdad scrollea el texto. Mismo
   * patrón que `registrarPegado`/`registrarEditor` de `EditorDePagina`.
   * OPCIONAL: la mitad derecha de la pantalla dividida no dibuja nada encima.
   */
  registrarHojaA4?: (el: HTMLDivElement | null) => void;
}) {
  return (
    <div className="pb-8">
      <div ref={registrarHojaA4} className={`hoja-a4 mx-auto ${dividida ? 'hoja-a4--dividida' : ''}`}>
        {children}
      </div>
    </div>
  );
