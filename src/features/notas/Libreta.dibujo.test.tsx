...
test('la herramienta Imagen está a la vista y tiene su buscador de archivos', async () => {
  ...
  // `aria-label`, y no un `input[type="file"]` genérico: desde `NuevaPagina.tsx`
  // (26-ago-2026, adjuntar un documento) la Libreta tiene MÁS de un buscador de
  // archivos, y un selector ambiguo tomaría cualquiera de los dos.
  const input = document.querySelector('input[aria-label="Elegir imagen"]') as HTMLInputElement | null;
...
test('🔴 el botón Imagen abre el buscador de archivos', async () => {
  await abrir('precios del diplomado');
  const input = document.querySelector('input[aria-label="Elegir imagen"]') as HTMLInputElement;
