...
export function VistaRouting() {
  ...
  /**
   * ⚠️ **Consulta aparte de la foto, y no por tamaño.** `useRouting` depende de
   * Meta y cambia cuando alguien refresca; el tablero cambia cada vez que cae un
   * lead. Con una sola clave, un lead entrante refrescaría también la parte cara
   * — y el SSE los invalida a ritmos distintos a propósito.
   */
  const tablero = useTableroDeRuteo();
  const ponerRegla = usePonerRegla();
  ...
  /**
   * ⚠️ **`ponerRegla` cuenta acá, y omitirlo abre el mismo agujero que el
   * `!guardando` vino a tapar.** Este flag congela la resiembra mientras un
   * guardado viaja; si no incluyera las reglas de producto, una foto vieja que
   * aterrice a mitad de un PUT borraría el cable recién tirado —y pisaría
   * `espejo.current`— exactamente como pasaba con los cables de campaña antes de
   * que existiera este freno.
   */
  const guardando =
    conectar.isPending ||
    conectarCurso.isPending ||
    conectarProducto.isPending ||
    ponerRegla.isPending;
   ...
   */
  /**
   * 🔴 **La división va PRIMERO en la cadena de elección**, antes que el
   * producto. Si fuera al revés, elegir una división caería en el fallback
   * `productosVisibles[0]` y la pantalla mostraría un producto cualquiera —
   * sin error, y con la sensación de que el clic no hizo nada.
   */
  const divisionElegida =
    (tablero.data?.divisiones ?? []).find((d) => ID.division(d.division) === elegido) ?? null;

  /**
   * ⚠️ **La huella incluye las reglas del TABLERO, no sólo las piezas.** Las
   * reglas de división y familia viven en otra consulta (`['routing','tablero']`)
   * y cambian por su cuenta: sin ellas acá, cablear una división guardaba bien y
   * el cable se quedaba dibujado en «pendiente» para siempre, porque la siembra
   * nunca se enteraba de que el server ya lo había confirmado.
   */
  const reglasDelTablero = [
    ...(tablero.data?.divisiones ?? []).map((d) => `d:${d.division}=${d.vendedoras.join(',')}`),
    ...(tablero.data?.familias ?? []).map((f) => `f:${f.familia}=${f.vendedoras.join(',')}`),
  ].join('|');
  ...
  useEffect(() => {
    ...
    const desdeElServer = [
      ...cablesDe(piezas, destinos),
      // Las reglas de producto son cables como cualquier otro para el lienzo:
      // lo que cambia es a qué endpoint van al soltarse, y eso lo decide `aplicar`.
      ...cablesDeDivision(
        { division: divisionElegida?.division ?? '', vendedoras: divisionElegida?.vendedoras ?? [] },
        tablero.data?.familias ?? [],
        destinos,
      ),
    ];
    ...
    /**
     * 🔴 **`elegido` ES UNA DEPENDENCIA, y faltaba.** El cable propio de la
     * división sale de `divisionElegida`, que deriva de `elegido`: sin él acá, al
     * elegir una división que YA tenía vendedoras la siembra no volvía a correr y
     * el lienzo dibujaba cero cables desde su nodo. El primer arrastre mandaba
     * entonces un conjunto de UNO a un `PUT` declarativo — **y el server borraba
     * a las que ya estaban**, mientras la fila de la izquierda las seguía
     * listando. Pérdida de datos silenciosa, y sólo en la primera vez por carga.
     */
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [huella, reglasDelTablero, (data?.destinos ?? []).join('|'), guardando, elegido]);
  ...
  /**
   * ⚠️ Se filtran con el MISMO `coincide` que el resto: buscar «inteligencia»
   * tiene que encontrar la división igual que encuentra sus familias, o la
   * búsqueda contestaría distinto según dónde esté lo que buscas.
   */
  const divisionesVisibles = (tablero.data?.divisiones ?? []).filter(
    (d) => coincide(d.nombre) && (filtro === 'todo' || filtro === 'division'),
  );
  ...
  const producto = divisionElegida
    ? null
    : (productoElegido ?? (piezaElegida ? null : (productosVisibles[0] ?? null)));
  const pieza = divisionElegida ? null : (piezaElegida ?? (producto ? null : (sueltasVisibles[0] ?? null)));
  ...
  const armado = divisionElegida
    ? columnasDeDivision(
        divisionElegida,
        (tablero.data?.familias ?? []).filter((f) => f.division === divisionElegida.division),
        data.destinos,
        data.deBaja ?? [],
      )
    : producto
    ? columnasDeProducto(producto, data.destinos, apertura, data.deBaja ?? [])
    : {
        columnas: pieza
          ? columnasDePieza(
              pieza,
  ...
  function aplicar(
    ...
    /**
     * 🔴 **UNA regla, no un abanico.** Al contrario del producto —que escribe un
     * cable por pieza porque no había regla que heredar—, cablear una división o
     * una familia escribe UNA fila en `producto_ruteo` y la cascada hace el
     * resto. Por eso `fam:` existe aparte de `prod:`: el mismo gesto sobre el
     * mismo diploma escribe en tablas distintas según desde dónde lo hagas, y
     * confundirlos no daría error — escribiría en la equivocada.
     */
    else if (tipo === 'div') ponerRegla.mutate({ eje: 'division', clave, vendedoras: destinos });
    else if (tipo === 'fam') ponerRegla.mutate({ eje: 'familia', clave, vendedoras: destinos });
  }
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      {/**
       * QUÉ REPARTIÓ EL RUTEO DE VERDAD. Va acá —debajo de los avisos y arriba
       * del lienzo— porque contesta la pregunta que se hace ANTES de tocar nada:
       * «¿lo que configuré está aplicándose?». Abajo del lienzo sería el pie de
       * una configuración que quizá no sirve.
       *
       * ⚠️ Se calla sola cuando no hay datos: ver el docblock de `Monitoreo`.
       */}
      <Monitoreo tablero={tablero.data} />
      ...
      <div className="relative flex min-h-0 flex-1">
        <aside className="flex w-[19rem] shrink-0 flex-col border-r border-border">
            ...
            <div className="flex flex-wrap gap-1">
              {(
                [
                  ...
                  ['division', 'Divisiones', (tablero.data?.divisiones ?? []).length],
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-3">
            ...
            sueltasVisibles.length === 0 &&
            divisionesVisibles.length === 0 ? (
              <p className="px-1 py-8 text-center text-[11px] text-muted-foreground">
                {/**
                 * ⚠️ **El cartel del vacío distingue TRES causas**, porque cada
                 * una la arregla otra persona: no llegó nadie todavía (esperar o
                 * refrescar Meta), no hay catálogo (falta refrescar Cerberus), y
                 * el filtro no encontró nada (cambiar el filtro). Un solo texto
                 * mandaría a revisar la pauta cuando lo que falta es un botón.
                 */}
                {piezas.length === 0 && (tablero.data?.divisiones ?? []).length === 0
                  ? 'No llegó nadie por esta línea y el catálogo de Cerberus todavía no se trajo. Prueba «Actualizar desde Meta».'
                  : piezas.length === 0
                    ? 'Ninguna campaña manda gente a esta línea y no llegaron formularios, pero el catálogo sí está: cablea una división para dejar la regla puesta antes del primer lead.'
                    : 'Nada coincide con el filtro.'}
              </p>
            ) : (
              <>
                {/**
                 * 🔴 **LAS DIVISIONES VAN PRIMERO, y no es orden alfabético.**
                 * Son TRES y cubren los 244 productos del catálogo (medido el
                 * 24-ago-2026); las familias son 185 y cubren una cada una. Lo
                 * que más rinde configurar tiene que ser lo primero que se ve —
                 * si Alex tiene que bajar para encontrarlas, va a cablear
                 * familias de a una, que es el trabajo que esto vino a evitar.
                 */}
                <Grupo titulo="Divisiones" mostrar={divisionesVisibles.length > 0}>
                  {divisionesVisibles.map((d) => (
                    <Fila
                      key={d.division}
                      titulo={d.nombre}
                      pie={`${d.productos} productos · ${d.familias} familias · ${
                        d.vendedoras.length ? d.vendedoras.join(', ') : 'sin cables'
                      }`}
                      activa={divisionElegida?.division === d.division}
                      onElegir={() => setElegido(ID.division(d.division))}
                    />
                  ))}
                </Grupo>
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          {/**
           * 🔴 **`divisionElegida` VA EN ESTA CONDICIÓN, y olvidarla dejó la
           * feature muerta.** `producto` y `pieza` se fuerzan a `null` cuando hay
           * división elegida, así que sin nombrarla acá el armado se calculaba y
           * se tiraba: la fila se marcaba activa a la izquierda y el lienzo
           * pintaba «Elige algo de la izquierda». La galería no lo desmintió
           * porque monta `Lienzo` DIRECTO, salteándose esta vista — que es la
           * lección de ADR 0024: un test de REGLA no reemplaza uno de CABLEADO.
           */}
          {divisionElegida || producto || pieza ? (
            <>
              <Lienzo
            ...
            <div className="flex min-h-0 flex-1 items-center justify-center p-6">
              <p className="text-xs text-muted-foreground">Elige algo de la izquierda para conectarlo.</p>
            </div>
          )}
        </div>
    ...
    <div className="flex shrink-0 flex-wrap items-center gap-x-3 gap-y-2 border-t border-border px-6 py-3">
      ...
      <p className="min-w-0 flex-1 text-[11px] leading-relaxed text-muted-foreground">
        {acuse
          ...
          : union.length === 0
            ? 'Tira un cable en alguna pieza y después puedes repetirlo en todas.'
      ...
      {huerfanos.length > 0 && (
        <p className="flex items-start gap-2 text-[11px] text-muted-foreground">
          ...
          <span>
            ...
            igual le caen. Para sacarlos, vuelve a conectar la pieza desde cero.
          </span>
        </p>
      )}
      {refrescar.isError && (
