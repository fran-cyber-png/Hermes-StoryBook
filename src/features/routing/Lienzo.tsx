...
import { PulsosDelLienzo } from './PulsosDelLienzo';
...
export function Lienzo({
  ...
  /**
   * DÓNDE ENTRA CADA NODO — su puerto izquierdo, en coordenadas del contenido.
   *
   * Lo necesita el puntito para aterrizar, y se mide acá y no allá por una razón
   * concreta: el que sabe dónde está cada puerto es este componente, que ya los
   * tiene registrados. Medirlos dos veces daría dos respuestas el día que una de
   * las dos se olvide de sumar el scroll — que es el defecto que `centroDe`
   * documenta.
   */
  const [anclas, setAnclas] = useState<Map<string, { x: number; y: number }>>(new Map());
   ...
   * dibujan corridos —y con ellos su área de clic, o sea que «toca el cable para
  ...
  useLayoutEffect(() => {
    function redibujar() {
      ...
      // Sólo los nodos de la ÚLTIMA columna: son los únicos donde aterriza un
      // lead. Medir los 185 de la lista de familias sería pagar 185
      // `getBoundingClientRect()` por redibujo para datos que nadie usa.
      const destinos = new Map<string, { x: number; y: number }>();
      for (const nodo of columnas[columnas.length - 1]?.nodos ?? []) {
        const p = centroDe(nodo.id, 'izq');
        if (p) destinos.set(nodo.id, p);
      }
      setAnclas(destinos);
    }
    redibujar();
  ...
  return (
    ...
    >
      ...
      <svg className="pointer-events-none absolute left-0 top-0 h-full w-full min-h-full min-w-full" aria-hidden>
        {trazos.map((t) => (
          <g key={t.clave}>
            {/* El trazo grueso invisible es el área de clic: 2 px de cable es
                imposible de acertar con el mouse, y sin esto «toca el cable para
        ...
        {/* Los puntitos van ADENTRO de este mismo svg y DESPUÉS de los trazos:
            así comparten el sistema de coordenadas del contenido (con el scroll
            ya sumado) y quedan por encima de los cables. Un svg propio
            `absolute inset-0` se recortaría apenas el lienzo desborde —el caso
            normal con diez vendedoras— y el puntito hacia la de abajo no se
            vería nunca. */}
        <PulsosDelLienzo trazos={trazos} anclas={anclas} />
      </svg>
