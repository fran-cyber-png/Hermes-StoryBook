...
export function usePasarConversacion() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      // 🔴 Y EL DASHBOARD, desde que su fila también asigna: sin esto, el chip
      // que se acaba de tocar sigue diciendo «Asignar» hasta el próximo latido
      // de 30 s — o sea que la acción se lee como que no pasó nada, y la
      // vendedora la repite. Es la MISMA foto de datos que la cola, servida por
      // otra ruta: invalidar una sola de las dos deja a la otra mintiendo.
