 ...
 *
 * Y son éstas y no `paso`/`dato`: el catálogo que Ivi consulta publica
 ...
 *
 * ⚠️ **`hsm` es la plantilla aprobada de Meta y NO es `plantilla`** (ADR 0062,
 * y el porqué está escrito en `server/src/piezas/direccion.ts`): `plantilla` es
 * la secuencia interna con sus pasos y su id numérico, la HSM vive en Meta y su
 * identidad es un nombre. Colapsarlas choca los dos espacios de ids, y una `ref`
 * de clase desconocida cae a la línea de base — que para una pieza que también
 * sale por campaña es veneno: contaminaría la fila contra la que se compara todo.
 */
export const CLASES_DECLARABLES = ['plantilla', 'hecho', 'hsm'] as const;
...
export interface PiezaDeclarada {
  ...
  ref: string;
  /**
   * Por qué superficie entró. Copia a mano de `VIAS_DE_PIEZA` (server), y
   * `procedencia/paridad.test.ts` falla si acá aparece una que allá no exista:
   * una vía desconocida la descarta `procedenciaDesdeColumnas` y el envío cae a
   * la línea de base, sin error y sin log.
   */
  via: 'panel-sugerencia' | 'panel-datos' | 'composer-hsm';
  editada: boolean;
  /**
