    ...
    <div className="mb-2 rounded-xl border border-success/30 bg-success/[0.06] p-2.5">
      <p className="mb-2 text-[11px] font-semibold text-foreground">
        ...
        <span className="font-normal text-muted-foreground">
          (era {pesoLegible(original.size)} · −{ahorro}%). Míralo antes de mandarlo.
        </span>
      </p>
      {url && (
        <video
...
/**
 * El bloque completo, desde «no entra» hasta «usa este».
 *
  ...
  async function comprimir(plan: Extract<Plan, { tipo: 'comprimir' }>) {
    ...
    try {
      ...
      if (salida.size > topeBytes) {
        setEstado({
          fase: 'fallo',
          motivo: `Quedó en ${pesoLegible(salida.size)} y sigue sin entrar. Recórtalo y prueba de nuevo.`,
        });
        return;
      }
      ...
      setEstado({ fase: 'fallo', motivo: 'No se pudo achicar el video acá. Prueba mandarlo más corto.' });
    } finally {
          ...
          {estado.fase === 'comprimiendo' && (
            <div className="mt-1.5">
              ...
              <p className="mt-1 text-[11px] text-muted-foreground">
                Pasa acá en tu compu, no se sube nada todavía. Puedes seguir escribiendo.
              </p>
            </div>
          )}
        </div>
