...
describe('ejecutarEnvioComposer', () => {
  it('camino de texto: manda, limpia el Map del teléfono del envío, y limpia el composer visible si sigues ahí', async () => {
    ...
    await ejecutarEnvioComposer({
      ...
      enviarTexto,
      enviarConAdjunto,
      telefonoVisibleAhora: () => '51999000010', // sigues viendo esa conversación
      limpiarTextoVisible,
  ...
  it('con adjunto: el texto viaja como caption, y limpia texto Y adjunto visibles si sigues ahí', async () => {
    ...
    expect(enviarConAdjunto).toHaveBeenCalledWith(archivo, 'mira esto'); // caption trimmeada
    expect(limpiarTextoVisible).toHaveBeenCalledTimes(1);
