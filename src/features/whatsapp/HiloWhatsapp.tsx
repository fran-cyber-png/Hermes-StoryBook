...
import { BotonPlantillas, FormularioHsmReal, SelectorPlantillas } from './SelectorPlantillas';
import { pegarPlantilla, usePlantillasAMano, type PlantillaAMano } from './plantillasAMano';
...
import { quienRegistro } from '../contactos/contactosRegistrados';
import { useFichaLocal } from '../panel/fichaLocal';
import { nombreDelContacto } from '../panel/identidad';
  ...
  return (
    <div className={'-mt-1.5 flex flex-wrap gap-1 px-1 ' + (saliente ? 'justify-end' : 'justify-start')}>
      {[...porEmoji].map(([emoji, { n, nuestra }]) => (
        <span
          key={emoji}
          title={nuestra ? 'Reaccionaste tú' : 'Reaccionó esta persona'}
          className={
...
export function HiloWhatsapp({
  ...
  /**
   * Quién está mirando, para saber si este chat es suyo (ADR 0083).
   *
   * ⚠️ **Ausente NO bloquea nada.** `mismaVendedora` con una cadena vacía da
   * `false`, así que sin este dato toda conversación con dueño se leería como
   * ajena y el compositor quedaría apagado — en la galería de piezas, en un
   * test que monta el hilo suelto y en cualquier consumidor que todavía no lo
   * pase. Por eso quien decide es `bloqueoDeChat`, que llega del server: sin él
  ...
  /**
   * EL NOMBRE DE LA CABECERA — misma jerarquía que `panel/identidad.ts`
   * (sin Cerberus acá, mismo motivo que `FilaConversacion.tsx`: no hay margen
   * para preguntarle al ERP por cada apertura). Antes esta cabecera SOLO
   * miraba `persona_nombre` (el pushname): un contacto de campaña recién
   * reclamado (`server/src/contactos/reclamar.ts`) abre el hilo con una
   * `Conversacion` armada en el cliente (`conversacionDeTelefono`), sin
   * `persona_nombre`, así que la cabecera seguía mostrando el teléfono pelado
   * aunque el nombre ya estuviera guardado y el panel derecho, al lado, ya lo
   * dijera bien (ampliación del 25-ago-2026).
   */
  const fichaDelHilo = useFichaLocal(conversacion.clave);
  const nombreDelHilo = nombreDelContacto({
    pushname: conversacion.persona_nombre,
    leadNombre: conversacion.lead_nombre,
    fichaNombre: [fichaDelHilo.data?.nombre, fichaDelHilo.data?.apellido].filter(Boolean).join(' ').trim() || null,
  }).principal;
  /**
   ...
   *
   * 🔴 **El reloj tiene que LATIR, y no es cosmético.** El vencimiento es una
   * fecha, no un evento: cuando pasan los 10 minutos el server no manda nada —no
   * hay cron, es la forma derivada de ADR 0073— así que sin un re-render por
   * minuto el compositor se queda apagado hasta que la persona toque algo. El
   * candado se levantaría solo en el server y la pantalla seguiría diciendo que
   * no. Un minuto de resolución alcanza: el rótulo habla en minutos.
   */
  ...
  const bloqueoActivo = useBloqueoDeChat();
  /**
   * 🔴 **LA TENENCIA SE LEE VIVA, NO DE `conversacion`.** Esa prop es una FOTO:
   * `App` la guarda en un `useState` al hacer clic en la fila y no se vuelve a
   * armar. Cuando otra persona toma este chat, la cola refresca sola —la fila de
   * la lista ya dibuja la píldora del dueño nuevo— pero acá seguiría llegando la
   * versión de hace diez minutos: caja habilitada, y quien escribe se come el
   * rechazo del server después de teclear. Medido con dos sesiones: a los 30 s
   * la pestaña seguía dejando escribir.
   *
   * ⚠️ **Se cae a la foto si la cola no tiene esta clave** (se filtró, salió del
   * recorte): ahí lo que traía la prop es lo último que se supo, y eso es mejor
   * que tratarla como libre.
   */
  const viva = useTenenciaViva(conversacion.clave);
  const tenencia = lecturaDeTenencia(
    { ...conversacion, ...(viva ?? {}) },
    { yo: miVendedora, activo: bloqueoActivo },
    ahora,
  );
  ...
  return (
    ...
    /**
     * EL CONTORNO VERDE = alguien más está atendiendo este chat (ADR 0083).
     *
     * ⚠️ **Sólo lo ve quien NO es el dueño** — `lecturaDeTenencia` devuelve
     * `null` para lo propio, igual que `marcaDeDueno` con la píldora de la fila.
     * La mayoría de los chats que alguien abre son suyos: un borde permanente
     * dejaría de ser una señal y pasaría a ser fondo.
     *
     * El verde va acá y el reloj en el rótulo, no en otro color: un solo color
     * para el candado, el detalle en el texto. (El oro de esta app significa
     * «tiempo que se acaba» y está tomado por la ventana de 24 h — dos cuentas
    ...
    <div
      ...
      className={
         ...
         *
         * `ring-*` de Tailwind compone `box-shadow`, y este panel ya lleva
         * `shadow-panel`, que en `index.css` declara `box-shadow` A PELO: gana la
         * declaración cruda y **el ring desaparece**. No hay error, no hay clase
         * faltante y `--tw-ring-color` queda bien puesta en el DOM — con
         * `ring-2 ring-inset ring-success` el elemento se veía idéntico a uno sin
         * candado. Ningún test puede ver esto: lo encontró la captura.
         *
         * `outline` es una propiedad aparte, así que no compite con la sombra, y
         * el offset negativo la dibuja adentro (el panel es `overflow-hidden` y
         * sus vecinos no dejan aire para una línea por fuera).
         */
    ...
    >
      ...
      */}
      ...
      {/* El hilo.
          ...
          para nada más: su `::before` es el patrón, y tiene que quedarse quieto
          mientras los mensajes scrollean. Puesto adentro del scroller se ataría
          al área desplazable y el patrón se iría hacia arriba con el texto.
          El que scrollea sigue siendo el de `data-hilo`, con su misma ref. */}
      {/* El color de fondo se mudó ACÁ y el scroller quedó transparente: con
          `bg-muted/30` encima, el patrón quedaba debajo de un velo y se perdía
          justo lo que se quería ver. */}
      <div className="fondo-hilo relative min-h-0 flex-1 bg-muted/30">
        <div ref={scrollRef} data-hilo className="relative h-full space-y-2 overflow-y-auto p-4">
        {hilo.isPending ? (
          <SkeletonHilo />
        ) : hilo.isError ? (
          <div className="py-8 text-center">
            <p className="text-sm text-muted-foreground">
              {esDeCampana(hilo.error)
                ? 'Esta conversación es de una línea de campaña y no la atiendes.'
                ...
                {g.items.map((m) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <div
                          className={
                            ...
                            (m.automatico || (m.direccion === 'saliente' && m.enviado_por)
                              ? 'justify-between '
                              : 'justify-end ') +
                            (m.media ? 'px-2 pb-1' : '')
                          }
                        >
                          ...
                          {/* 🔴 **QUIÉN CONTESTÓ.** Una línea la atienden varias
                              personas —la de campaña, DIECIOCHO— y el hilo no
                              decía nada: quien abría el chat tres días después no
                              tenía cómo saber quién había respondido. Pedido del
                              dueño: «que se sepa quién respondió a tal dato para
                              tenerlo mapeado».
                              ⚠️ **No se dibuja si el mensaje ya es automático**:
                              ahí el `vendedora_id` es el del despachador, y dos
                              etiquetas en la misma línea dicen lo mismo dos veces.
                              ⚠️ Y **ausente NO se dibuja**: falta en lo que se
                              mandó desde el teléfono de la vendedora (no pasa por
                              `envios_wa`) y en lo anterior a este frente. Un
                              nombre inventado es peor que un hueco.
                              🔴 **Y pide `direccion === 'saliente'` explícito**:
                              la autoría es de lo que MANDAMOS. Un entrante no
                              debería traer el campo —el join va por
                              `external_id` contra `envios_wa`— pero el cluster
                              de la hora se dibuja para los dos sentidos, así que
                              sin la guarda un entrante con ese dato le atribuía
                              a alguien del equipo un mensaje del lead. Lo
                              encontró el test, no la lectura. */}
                          {m.direccion === 'saliente' && !m.automatico && m.enviado_por && (
                            <span
                              className="font-sans text-[10px] font-medium text-muted-foreground"
                              title={`Lo mandó ${m.enviado_por} desde Hermes`}
                            >
                              {quienRegistro(m.enviado_por)}
                            </span>
                          )}
                          <span className="inline-flex items-center gap-1">
                            ...
                            {/* 🔴 LA BURBUJA PROVISIONAL NO LLEVA ✓, Y ESO ES EL
                                CONTRATO. Desde que `enviar` es optimista, el
                                mensaje aparece antes de haber salido; si además
                                mostrara un tilde, diría «llegó» sobre algo que
                                todavía está en la fila del pool. Un mensaje
                                dibujado como enviado que no salió es peor que
                                esperar. Mismo vocabulario que ADR 0058. */}
                            {m.enVuelo === 'enviando' && (
                              <span className="italic opacity-70" title="Todavía saliendo de Hermes">
                                enviando…
                              </span>
                            )}
                            {m.enVuelo !== 'enviando' && m.entrega && (
                              <TildesDeEntrega estado={m.entrega} motivo={m.entregaMotivo} />
                            )}
                          </span>
                        </div>
                        )}
                      ...
                      {/* No se pudo ni MANDAR (la request falló), que no es lo
                          mismo que «salió y Meta lo rechazó» —el caso de arriba—.
                          La burbuja se queda en pantalla a propósito: quitarla
                          borraría el texto que la vendedora escribió. */}
                      {m.enVuelo === 'fallido' && (
                        <p className="mt-1 text-right text-[11px] leading-snug text-destructive">
                          No se pudo enviar. El texto sigue acá: cópialo e intenta de nuevo.
                        </p>
                      )}
                      </div>
      ...
      <ComposerWa
        ...
        enviarPlantillaHsm={enviarPlantillaHsm}
        // ADR 0072: la MISMA lectura que `AvisoDeVentana` de arriba —cerrada
        // solo en Cloud API, con el reloj de este render—, precalculada acá
        // para no sumarle a `ComposerWa` el `ventanaCierra` + `transporte`
        // crudos que ya tiene su propio consumidor (la doc de la línea 1216).
        ventanaCerrada={avisoDeComposer(conversacion.ventana_cierra, sesion?.transporte, new Date())?.clase === 'cerrada'}
        puedeMandarPlantilla={Boolean(sesion?.puedeMandarPlantilla)}
        /* ADR 0083: quién lo está atendiendo, ya resuelto acá — el composer no
           vuelve a juzgar nada. Es el mismo criterio que `ventanaCerrada`. */
        tomadoPor={tenencia?.nombre ?? null}
      />
      ...
      {arrastrandoArchivo && (
        ...
        >
          ...
          <p className="px-8 text-center font-heading text-base font-bold text-foreground">
            {sugerencia ? 'Acá no se adjunta' : 'Suelta el archivo acá'}
          </p>
          <p className="max-w-xs px-8 text-center text-xs text-muted-foreground">
            {sugerencia
              ...
              : `Se lo mandas a ${conversacion.persona_nombre ?? formatoTelefono(telefono)}, con tu nombre.`}
          </p>
        </div>
      )}
    </div>
...
type MutacionEnviarPlantillaHsm = ReturnType<typeof useConversacionWa>['enviarPlantillaHsm'];
...
function ComposerWa({
  ...
  enviarPlantillaHsm: MutacionEnviarPlantillaHsm;
  sugerencia?: SugerenciaEnComposer;
  /**
  ...
  /**
   * ¿LA VENTANA DE 24 H YA CERRÓ? (ADR 0072). Precalculada arriba con la MISMA
   * lectura que `AvisoDeVentana` — acá no se recalcula, solo se usa para
   * decidir si el selector de plantillas ofrece «enviar como plantilla real»
   * en vez de (o además de) pegar el texto.
   */
  ventanaCerrada?: boolean;
  /** ¿Esta línea puede mandar una HSM real? Feature-detectado (`GET /sesion`). */
  puedeMandarPlantilla?: boolean;
  /**
   * QUIÉN ESTÁ ATENDIENDO ESTE CHAT, si no eres tú (ADR 0083). `null` = puedes
   * escribir. Precalculado arriba con `lecturaDeTenencia`, igual que
   * `ventanaCerrada`: acá no se vuelve a juzgar nada.
   *
   * ⚠️ **Apagar la caja es cortesía, NO la garantía.** El que rechaza el envío
   * es el server (`whatsapp/enviarYProyectar.ts`); esto sólo evita que alguien
   * escriba tres párrafos para comerse un 409. Es la misma distinción que
   * `avisoDeComposer` hace con la ventana de 24 h, con el signo cambiado: allá
   * se avisa y no se bloquea porque el dato puede estar incompleto; acá se
   * bloquea porque del otro lado hay un candado de verdad que dice lo mismo.
   */
   ...
   *
   * `hasta` permite dejar un tramo SELECCIONADO en vez de solo el cursor: lo usa
   * el pegado de una plantilla con huecos (`{{1}}`), donde lo primero que tiene
   * que pasar es que ella lo reemplace — y algo seleccionado se reemplaza
   * tipeando, mientras que un cursor al lado hay que darse cuenta de que está.
   */
  function reemplazarEnLaCaja(nuevo: string, posicion: number, hasta = posicion) {
    setTexto(nuevo);
    setCursor(hasta);
  ...
  /**
   * ── LAS PLANTILLAS APROBADAS, AL LADO DEL CLIP (ADR 0062) ────────────────
   *
   * El catálogo se pide **recién al abrir** (`enabled`): detrás hay una request a
   * la Graph API con 15 s de techo, y pagarla al montar el composer sería pagarla
   * cada vez que se abre una conversación, que es la acción más frecuente del día.
   *
   * ⚠️ **En modo revisión no existe**, igual que el `/`: ahí la caja no es para
   * escribir sino para aprobar un texto que ya salió preparado, y meterle una
   * plantilla lo convierte en otra cosa sin que quede registro de qué se aprobó.
   */
  const [plantillasAbiertas, setPlantillasAbiertas] = useState(false);
  const [consultaPlantilla, setConsultaPlantilla] = useState('');
  const [indicePlantilla, setIndicePlantilla] = useState(0);
  const catalogoPlantillas = usePlantillasAMano(plantillasAbiertas && !sugerencia);
  // El clic afuera y el Escape con el foco FUERA del buscador. El Escape con el
  ...
  function elegirPlantilla(p: PlantillaAMano) {
    const r = pegarPlantilla(texto, cursor, p.cuerpo);
    reemplazarEnLaCaja(r.texto, r.desde, r.hasta);
    setPlantillasAbiertas(false);
    setConsultaPlantilla('');
    /**
     * LA PROCEDENCIA (ADR 0022). La clase es `hsm` —la plantilla vive en Meta y
     * su identidad es un nombre—, y **la vía la separa de `campana`**: la misma
     * pieza mandada a mil personas en frío y pegada en un chat que ya venía
     * conversando no rinde igual, y sumadas en una fila el promedio esconde justo
     * lo que se quiere ver.
     *
     * Se anota el cuerpo APROBADO, que es lo que el server hashea para la versión
     * (`procedencia/desdeElComposer.ts`): así esta fila casa con las de la campaña
     * que mandó la misma plantilla. ⚠️ Si ella completa un hueco, el texto deja de
     * contener el cuerpo tal cual y `piezaDelTexto` cae a la línea de base — «ante
     * la duda, a mano», que es la política del módulo y no un olvido.
     */
    anotarPieza(telefono, { clase: 'hsm', ref: p.nombre, via: 'composer-hsm' }, p.cuerpo);
  }

  /**
   * ── ENVIAR COMO PLANTILLA REAL (ADR 0072) ────────────────────────────────
   *
   * Un camino DISTINTO al de arriba: no pasa por la caja, no pasa por
   * `anotarPieza` (la procedencia la calcula el server entero,
   * `deUnaHsmDelComposer`) y manda directo por `POST /enviar-plantilla`. Por
   * eso vive en su propio estado, separado de `plantillasAbiertas`.
   */
  const [plantillaHsmReal, setPlantillaHsmReal] = useState<PlantillaAMano | null>(null);

  function abrirEnvioHsmReal(p: PlantillaAMano) {
    setPlantillasAbiertas(false);
    setConsultaPlantilla('');
    setPlantillaHsmReal(p);
    enviarPlantillaHsm.reset();
  }

  function confirmarEnvioHsmReal(input: { variables: string[]; imagen: File | null }) {
    if (!plantillaHsmReal) return;
    enviarPlantillaHsm.mutate(
      {
        numeroPropio,
        telefono,
        referencia: conversacionClave,
        nombrePlantilla: plantillaHsmReal.nombre,
        idioma: plantillaHsmReal.idioma,
        variables: input.variables,
        imagen: input.imagen,
      },
      { onSuccess: () => setPlantillaHsmReal(null) },
    );
  }
  ...
  useEffect(() => {
    ...
    /** Lo que el navegador abriría solo si lo sueltas: un archivo o una URL. */
     ...
     * contestaría un consejo equivocado («bájalo y arrástralo desde la carpeta» no
    ...
    function alSoltar(e: DragEvent) {
      ...
      if (!traeArchivos(e)) {
        ...
        setAvisoPegado('Eso vino de una página web y no trae el archivo. Bájalo primero y arrástralo desde la carpeta.');
        return;
      }
  ...
  return (
    ...
    >
      ...
      <div className={'relative ' + (sugerencia ? 'flex flex-col gap-2' : 'flex items-end gap-2')}>
        ...
        {/* ⚠️ El cajón de plantillas NO se dibuja mientras hay un `/` en curso:
        ...
        {plantillasAbiertas && !comando && (
          <>
            ...
            <SelectorPlantillas
              consulta={consultaPlantilla}
              onConsulta={setConsultaPlantilla}
              indice={indicePlantilla}
              onIndice={setIndicePlantilla}
              onElegir={elegirPlantilla}
              onCerrar={() => setPlantillasAbiertas(false)}
              catalogo={catalogoPlantillas.data}
              cargando={catalogoPlantillas.isPending && catalogoPlantillas.fetchStatus !== 'idle'}
              error={catalogoPlantillas.error}
              // Solo con la ventana YA cerrada: con la ventana abierta, pegar
              // como texto es más flexible (se edita antes de mandar) y ya
              // sirve — ofrecer las dos a la vez es superficie sin uso.
              puedeEnviarComoReal={ventanaCerrada && puedeMandarPlantilla}
              onEnviarComoReal={abrirEnvioHsmReal}
            />
          </>
        )}
        {plantillaHsmReal && (
          <>
            <div className="fixed inset-0 z-20" onClick={() => setPlantillaHsmReal(null)} />
            <FormularioHsmReal
              plantilla={plantillaHsmReal}
              enviando={enviarPlantillaHsm.isPending}
              error={enviarPlantillaHsm.error ? enviarPlantillaHsm.error.message : null}
              onCancelar={() => setPlantillaHsmReal(null)}
              onEnviar={confirmarEnvioHsmReal}
            />
          </>
        )}
        <input
        ...
        {/* AL LADO DEL CLIP, y no en el panel derecho: las dos son «traer algo de
            afuera a la caja», y el gesto que la vendedora ya tiene aprendido para
            eso es mirar a la izquierda de donde escribe.

            ⚠️ Se esconde en revisión como el clip, y se apaga con la sesión
            caída: pegar un texto que no se va a poder mandar es prometer algo
            que no existe. */}
        {!sugerencia && (
          <BotonPlantillas
            abierto={plantillasAbiertas}
            onAlternar={() => setPlantillasAbiertas((v) => !v)}
            disabled={!conectado || Boolean(tomadoPor)}
          />
        )}
        ...
        <textarea
          ...
          disabled={(!conectado && !sugerencia) || Boolean(tomadoPor)}
          ...
          /**
           * En revisión la caja arranca ALTA (ver arriba). En el composer normal
           * arranca en UN renglón… salvo que lo que hay adentro tenga varios:
           * ahí se abre a cuatro, con el tope de `max-h-28` de siempre.
           *
           * 🔴 Lo mostró la captura de ADR 0062: una plantilla pegada son diez
           * líneas, y en un renglón y medio **no se puede leer lo que se está por
           * mandar**. La alternativa —scrollear adentro de una caja de 40 px— es
           * la forma más rápida de mandar sin leer, que es exactamente lo que el
           * modo revisión ya había aprendido a no hacer.
           */
          rows={sugerencia ? 5 : texto.includes('\n') ? 4 : 1}
          placeholder={
            // 🔴 El chat ajeno va PRIMERO: con la sesión caída y el chat tomado,
            // «La sesión no está conectada» manda a la persona a revisar el
            // teléfono cuando el problema es otro y no lo arregla ella.
            tomadoPor
              ? `${tomadoPor} está atendiendo esta conversación`
              : !conectado
      ...
      <p className={'mt-1.5 text-center text-[11px] text-muted-foreground' + (sugerencia ? '' : ' max-md:hidden')}>
        {sugerencia ? (
          ...
          <>Se envía solo a esta persona, con tu nombre. Nada masivo: lo que escribes acá lo mandas tú.</>
        )}
      </p>
    </footer>
  );
...
function BannerSesion({ sesion }: { sesion: EstadoSesionWa | undefined }) {
  ...
  if (sesion.estado === 'sin-vincular') {
    return (
      <div className="flex items-center gap-2 border-b border-border bg-secondary px-4 py-2.5 text-xs text-secondary-foreground">
        <QrCode size={14} className="shrink-0" />
        <b>Número sin vincular.</b>
      </div>
    );
  }
