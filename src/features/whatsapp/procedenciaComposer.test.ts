...
describe('qué pieza declarar para lo que hay en la caja', () => {
  ...
  test('reescribió todo: NO hay pieza, el envío cuenta como escrito a mano', () => {
    anotarPieza(TEL, DATO, FRASE);
    expect(piezaDelTexto(TEL, 'puedes pagarlo en dos veces si quieres')).toBeUndefined();
  });
  ...
  test('la última pieza puesta gana: poner un dato y después otro no acumula', () => {
    ...
    anotarPieza(TEL, otro, 'El acceso lo tienes por todo un año.');
    expect(piezaDelTexto(TEL, 'El acceso lo tienes por todo un año.')).toEqual({
      ...otro,
      editada: false,
      textoPieza: 'El acceso lo tienes por todo un año.',
    });
  });
