...
export function planDeCompresion(video: EntradaVideo, topeBytes: number): Plan {
  ...
  return {
    ...
    motivo:
      minutos >= 2
        ? `Dura ${minutos} minutos: para que entre habría que dejarlo ilegible. Recórtalo y vuelve a intentar.`
        : 'No entra sin dejarlo ilegible. Recórtalo y vuelve a intentar.',
  };
}
