...
describe('el hilo se pide según qué tan viva está la conversación', () => {
  ...
  /**
   * 🔴 Y CON EL STREAM VIVO, EL ESCALÓN FRÍO SE ESPACIA A 5 MINUTOS (PR 4 de
   * `docs/plan-borrar-el-polling.md` §6): los ✓✓ de la Cloud API ya avisan
   * por el bus (`entrega/avisoDebounced.ts`), así que el poll deja de ser el
   * único camino para que aparezca un tilde.
   */
  it('con el stream vivo, una conversación fría se repregunta cada 5 minutos', async () => {
    marcarLatido();
    const espia = servidor(() => json(hiloConUltimoDe(30 * 24 * 60 * 60_000)));
    montado = montar(<AnfitrionHilo />);
    await correrElReloj(0);
    ...
    // Un solo escalón de 5 min en un intervalo de 5 min: inicial + 1.
    expect(pedidos(espia, '/api/whatsapp/conversacion/')).toBe(2);
  });
...
describe('el estado de la línea', () => {
  ...
  /**
   * 🔴 Y CON EL STREAM VIVO, LA RED SE ESPACIA A 5 MINUTOS (PR 1 de
   * `docs/plan-borrar-el-polling.md` §6). El poll de 60 s ya era la red por
   * ESCRITO del evento `estado` (`lib/datos/tiempoReal.ts` invalida
   * `['wa','sesion']` en cuanto llega uno) — acá se verifica el CABLEADO real:
   * que `useSesionWa` de verdad lea `streamVivo()`, no sólo que la función pura
   * lo sepa (eso ya está en `cadencia.test.ts`).
   */
  it('con el stream vivo, la sesión se repregunta cada 5 minutos', async () => {
    marcarLatido();
    const espia = servidor(() => json({ estado: 'conectado', telefono: LINEA }));
    montado = montar(<AnfitrionSesion linea={LINEA} />);
    await correrElReloj(0);
    ...
    // Un solo escalón de 5 min en un intervalo de 5 min: inicial + 1.
    expect(pedidos(espia, '/api/whatsapp/sesion')).toBe(2);
  });
