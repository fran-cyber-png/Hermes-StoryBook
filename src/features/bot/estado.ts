...
/**
 * LOS QUE SE PUEDEN ELEGIR — el RESPALDO cuando el server no lo dice (ADR 0077).
 *
 * La fuente es el server: `GET /api/bot/estado` manda `elegible` por modo y
 * `elegiblesDe()` lo lee. Esta lista sólo vale para un server VIEJO que no manda
 * la bandera — y ahí se ofrecen los tres, que es lo que ese server acepta. Tener
 * acá la política nueva mientras el server viejo sigue aceptando `automatico`
 * sería dibujar una puerta cerrada sobre una que está abierta.
 */
export const MODOS_BOT_ELEGIBLES_RESPALDO: readonly ModoBot[] = MODOS_BOT;
...
export interface RespuestaBotApi {
  ...
  /** `elegible` llegó con ADR 0077: `false` = existe (una fila puede tenerlo) pero no se puede ir ahí. */
  modos?: { modo: string; descripcion: string; elegible?: boolean }[];
}
...
export interface VistaBot {
  ...
  /**
   * A DÓNDE SE PUEDE IR (ADR 0077). Los segmentos del selector son éstos; el
   * modo puesto que NO esté acá se dibuja marcado y sin poder elegirse, y el
   * rótulo dice «retirado».
   */
  elegibles: readonly ModoBot[];
}
...
export function verBot(datos: RespuestaBotApi | undefined, ctx: Contexto = {}): VistaBot {
  ...
  const elegibles = elegiblesDe(datos);
  ...
  if (!esModoBot(datos.modoEfectivo)) {
    return {
      ...
      detalle:
        ...
        'No se puede afirmar si el bot está mandando o no. Mira el server antes de tocar nada.' +
  ...
  // Un modo puesto que ya no se puede ELEGIR (ADR 0077: `automatico`) se dice:
  // el selector lo muestra marcado y sin tocar, y este rótulo explica por qué
  // no es un destino. No se colapsa a sombra — el server lo degrada al mandar
  // (orquestador, paso 5), pero la fila dice lo que dice.
  const retirado = !elegibles.includes(modo);
  ...
  return {
    ...
    aviso: retirado ? 'retirado' : null,
    detalle:
      `${queHace[modo]} ${fuente}.` +
      (retirado
        ? ` El modo «${NOMBRE_MODO[modo]}» está RETIRADO (ADR 0077): el server lo trabaja como sombra y desde acá sólo se puede ir a ${elegibles.map((m) => NOMBRE_MODO[m]).join(' o ')}.`
        : ''),
    puedeCambiar: true,
  };
}
...
/**
 * A dónde se puede ir, según el SERVER. Si manda `elegible` en al menos un modo,
 * manda la lista (los que digan `true`); si no la manda —server anterior a
 * ADR 0077— valen los tres, que es lo que ese server acepta en el `PUT`.
 */
export function elegiblesDe(datos: RespuestaBotApi): readonly ModoBot[] {
  const filas = (datos.modos ?? []).filter((f) => typeof f.elegible === 'boolean');
  if (filas.length === 0) return MODOS_BOT_ELEGIBLES_RESPALDO;
  const lista = filas.filter((f) => f.elegible === true && esModoBot(f.modo)).map((f) => f.modo as ModoBot);
  // Una lista vacía sería «no se puede ir a ningún lado», y eso deja el kill-switch
  // sin «Apagado»: se cae al respaldo y que el server conteste lo que conteste.
  return lista.length > 0 ? lista : MODOS_BOT_ELEGIBLES_RESPALDO;
}
