...
import { NOMBRE_MODO, puertaDelFreno, type ClaseBot, type ModoBot, type VistaBot } from './estado';
...
/**
 * LOS DESTINOS, a la vista (desde ADR 0077, los ELEGIBLES: hoy apagado y sombra —
 * ver `destinosDe`). Es un `radiogroup` de verdad (no tres
...
function SelectorDeModo({
  ...
  return (
    ...
    >
      {destinosDe(vista).map(({ modo: m, retirado }) => {
        ...
        return (
          <button
            ...
            role="radio"
            aria-checked={activo}
            // Un destino retirado (ADR 0077) se ve porque está PUESTO, pero no
            // se puede volver a elegir: no es a dónde ir, es dónde se está.
            disabled={deshabilitado || activo || retirado}
            title={retirado ? `${vista.queHace[m]} — RETIRADO (ADR 0077): no se puede elegir` : vista.queHace[m]}
            onClick={() => onElegir(m)}
            className={
...
/**
 * QUÉ SEGMENTOS SE DIBUJAN (ADR 0077): los elegibles que manda el server y, si el
 * modo puesto no está entre ellos (una fila vieja en `automatico`), ése también
 * —marcado y deshabilitado— para que el control diga dónde se está sin ofrecer
 * volver ahí. Antes eran los tres siempre; ahora «los tres» es lo que dibuja un
 * server viejo, que es lo que ese server acepta.
 */
function destinosDe(vista: VistaBot): { modo: ModoBot; retirado: boolean }[] {
  const lista = vista.elegibles.map((m) => ({ modo: m, retirado: false }));
  if (vista.modo && !vista.elegibles.includes(vista.modo)) lista.push({ modo: vista.modo, retirado: true });
  return lista;
}
