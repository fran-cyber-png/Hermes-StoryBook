...
import { deDondeSaleElModo, porQueNoSeGuardo, puertaDelFreno, QUE_HACE, verBot, type RespuestaBotApi, elegiblesDe } from './estado';
...
describe('a dónde se puede ir (ADR 0077)', () => {
  it('lo dice el server: sólo los `elegible: true`', () => {
    expect(
      elegiblesDe({
        modos: [
          { modo: 'apagado', descripcion: '', elegible: true },
          { modo: 'sombra', descripcion: '', elegible: true },
          { modo: 'automatico', descripcion: '', elegible: false },
        ],
      }),
    ).toEqual(['apagado', 'sombra']);
  });

  it('un server viejo (sin la bandera) ofrece los tres: es lo que ese server acepta', () => {
    expect(elegiblesDe({})).toEqual(['apagado', 'sombra', 'automatico']);
    expect(elegiblesDe({ modos: [{ modo: 'sombra', descripcion: 'x' }] })).toEqual(['apagado', 'sombra', 'automatico']);
  });

  it('una lista vacía NO deja el kill-switch sin «Apagado»: cae al respaldo', () => {
    expect(elegiblesDe({ modos: [{ modo: 'sombra', descripcion: '', elegible: false }] })).toEqual([
      'apagado',
      'sombra',
      'automatico',
    ]);
  ...
  it('un modo puesto que no es elegible se dice «retirado» en la vista', () => {
    const v = verBot({
      numero: '51984429504',
      habilitada: true,
      modoEfectivo: 'automatico',
      modoDeLaBase: 'automatico',
      modoDelEntorno: 'sombra',
      frenado: false,
      frenadoMotivo: null,
      modos: [
        { modo: 'apagado', descripcion: '', elegible: true },
