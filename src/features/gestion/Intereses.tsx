...
export function Intereses({
  ...
  /**
   * El buscador se abre pegado al lado derecho del botón «+ interés», en vez de
   * debajo de toda la fila. Solo para `BarraGestion` (barra ancha, con lugar a
   * la derecha): `compacto` también lo usan `FichaRapida` y el panel derecho
   * —360 px, SIN lugar a la derecha—, así que esto no puede ir atado a
   * `compacto` sin romper esos dos.
   */
  abrirALaDerecha?: boolean;
}) {
    ...
    <div className={(compacto ? '' : 'mt-1') + (resaltado ? ' rounded-md ring-2 ring-primary' : '')}>
      <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1">
        ...
        <span className="relative inline-block">
          ...
          {/* Pegado al botón, hacia la derecha — solo cuando hay lugar (ver el
              porqué de `abrirALaDerecha` arriba). */}
          {abierto && abrirALaDerecha && (
            <div
              className="absolute left-full top-0 z-20 ml-1.5 w-64"
      ...
      {/* Debajo de toda la fila — el comportamiento de siempre, para
          `FichaRapida` y el panel derecho (360 px, sin lugar a la derecha). */}
      {abierto && !abrirALaDerecha && (
...
/** El input + la lista de sugerencias — el mismo contenido, en dos ubicaciones posibles. */
function BuscadorDeInteres({
  inputRef,
  q,
  setQ,
  idx,
  ...
  sugs: { id: string; nombre: string }[];
  agregar: { mutate: (v: { curso: string; productoId?: string }) => void };
  onEscape: () => void;
}) {
  return (
    <>
