...
import { AlertTriangle, BadgeCheck, ExternalLink, GraduationCap, Megaphone, MessageCircle, Search, ShoppingBag, ShoppingCart, UserPlus, Users2 } from 'lucide-react';
...
/** Píldora de estado de una venta — mismo par bg/texto que la ficha del panel. */
function pillEstadoVenta(estado: string): string {
  const e = estado.toLowerCase();
  if (e.includes('pagad') || e.includes('complet')) return 'bg-success/10 text-success';
  if (e.includes('anulad') || e.includes('cancelad')) return 'bg-destructive/10 text-destructive';
        ...
        >
          ...
          {faltanDigitos && (
            <p className="mt-2 text-[11px] text-muted-foreground">
              Faltan dígitos — incluye el código de país (51 …).
            </p>
          )}
          ...
          {!buscando && (
            <div className="mt-10 flex flex-col items-center gap-3 text-center">
              ...
              <p className="max-w-sm text-sm text-muted-foreground">
                ¿Te dictaron un número o llegó por Messenger sin teléfono? Pégalo arriba: te digo si
                        ...
                        <li key={v.folio} className="rounded-xl border border-border bg-muted/40 px-3 py-2 text-xs">
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex min-w-0 items-center gap-1.5">
                              <span className="shrink-0 font-mono font-semibold text-foreground">{v.folio}</span>
                              <span
                                className={`shrink-0 rounded px-1.5 py-[1px] text-[10px] font-bold ${pillEstadoVenta(v.estado)}`}
                              >
                                {v.estado}
                              </span>
                            </div>
                            <span className="shrink-0 font-mono font-bold tabular-nums text-navy-ink">
                              {v.moneda} {v.monto}
                            </span>
                          </div>
                          <div className="mt-1 flex items-center justify-between gap-2">
                            {v.productos.length > 0 ? (
                              <span className="flex min-w-0 items-center gap-1 text-[11px] text-foreground/80">
                                <GraduationCap size={11} className="shrink-0 text-muted-foreground" />
                                <span className="truncate">{v.productos.join(' · ')}</span>
                              </span>
                            ) : (
                              <span />
                            )}
