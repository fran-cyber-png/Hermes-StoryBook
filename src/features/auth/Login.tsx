 ...
 * pantalla decía «revisa tu internet» — mandando a reiniciar el router a alguien
...
function mensajeDeError(err: unknown): string {
  if (err instanceof ErrorApi && err.status === 401) {
    return 'Usuario o contraseña incorrectos. Revisa y vuelve a intentar.';
  }
  if (err instanceof ErrorApi && err.tipo === 'cerberus_caido') {
    return 'Cerberus no responde. Espera un minuto y prueba de nuevo; si sigue, avisa a sistemas.';
  }
  if (err instanceof ErrorApi && err.tipo === 'sin_linea_asignada' && err.message) {
    return err.message;
  }
  return 'No pude conectar con Hermes. Revisa tu internet y prueba de nuevo; si sigue, avisa a sistemas.';
}
