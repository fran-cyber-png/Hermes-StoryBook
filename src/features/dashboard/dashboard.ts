...
export interface LeadChat {
   ...
   * vencida se lee como `null`). Es lo que deja que el chip de asignar de la
   * fila diga a quién está asignada en vez de ofrecer «Asignar» sobre algo que
   * ya tiene dueña.
   *
   ...
   * (ADR 0007). Ausente y `null` se dibujan igual —sin dueña— así que el peor
   * caso es el chip de antes, nunca un hueco.
   */
...
export function useDashboard({ activa = true, vivo = false }: OpcionesDashboard = {}) {
  return useQuery({
    ...
    enabled: activa,
    /**
     * Red de seguridad del radar; el SSE lo invalida al instante (agrupado,
     * `lib/datos/tiempoReal.ts`). `false` y no un número grande cuando no está
     * `vivo`: un intervalo apagado no programa un timer.
     *
     * ⚠️ **Dos «vivo» distintos, y no se confunden**: el `vivo` del parámetro
     * es «¿esta pantalla quiere el latido?» (una vista de radar sí, un recibo
     * que se lee una vez no); `streamVivo()` es «¿el push está conectado?». El
     * segundo decide CUÁNTO, nunca SI — sin él acá, cada radar abierto pedía
     * cada 30 s aunque el SSE ya lo estuviera empujando al instante
     * (`docs/plan-borrar-el-polling.md` §6 PR 1).
     */
...
export function conversacionDeChat(c: LeadChat): Conversacion {
  return {
    ...
    // De quién es: sin esto el chip de asignar de la fila del radar ofrecería
    // «Asignar» sobre una conversación que ya tiene dueña — y la hoja que se
    // abre desde el radar diría lo mismo que la fila, o sea nada.
    asignada_a: c.asignada_a ?? null,
    texto: c.texto,
