...
describe('cuántos filtros están puestos', () => {
  ...
  test('suma «asignado a» y el rango de fecha', () => {
    expect(contarActivos({ asignadoA: ['luz', 'tracy'] })).toBe(2);
    expect(contarActivos({ entroDesde: '2026-08-01' })).toBe(1);
    expect(contarActivos({ entroDesde: '2026-08-01', entroHasta: '2026-08-24' })).toBe(1);
  });
});
