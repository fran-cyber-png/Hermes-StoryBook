...
describe("motivoParaNoProbar", () => {
  ...
  it("un mensaje en blanco no se manda", () => {
    expect(motivoParaNoProbar({ mensaje: "   ", esperando: false, linea: "5198" })).toMatch(/Escribe/);
  });
  ...
  it("mientras espera no se puede mandar otro", () => {
    expect(motivoParaNoProbar({ mensaje: "hola", esperando: true, linea: "5198" })).toMatch(/Espera/);
  });
