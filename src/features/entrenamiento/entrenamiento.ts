...
export function motivoParaNoProbar(entrada: {
  ...
  if (entrada.esperando) return "Espera la respuesta anterior.";
  if (!entrada.linea) return "Elige contra qué línea probar.";
  const limpio = entrada.mensaje.trim();
  if (limpio.length === 0) return "Escribe un mensaje.";
