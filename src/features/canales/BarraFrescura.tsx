...
export default function BarraFrescura() {
  ...
  // Calculado AHORA, no leído de un campo que el server hubiera cacheado: con
  // el caché sin vencer, el mismo cuerpo sirve horas y este número tiene que
  // seguir avanzando (docs/plan-borrar-el-polling.md §6 PR 2).
  const horas = horasDesdeIngesta(data);
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        {data.ultimaIngesta == null ? 'Nunca se capturaron datos' : `Captura detenida ${hace(horas)}`}
      </button>
        ...
        <>
          ...
          <div className="absolute right-0 top-full z-50 mt-2 w-72 rounded-xl bg-card p-4 shadow-panel animate-entrar">
            <p className="text-xs font-bold text-foreground">
              {data.ultimaIngesta == null ? 'Nunca se capturaron datos.' : `La captura está detenida ${hace(horas)}.`}
            </p>
            <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
              Los mensajes nuevos de Facebook e Instagram no están entrando. Avisa a sistemas.
              {dias != null && dias >= 7 && (
                <>
                  {' '}
