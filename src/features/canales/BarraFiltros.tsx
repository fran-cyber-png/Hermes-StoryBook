          ...
          >
            ...
            {opciones.map((l) => {
              ...
              return (
                ...
                >
                  {l.etiqueta}
                  {/* POR DÓNDE SALE ESA LÍNEA. Va acá y no en el semáforo del
                      header porque ese elige «la primera conectada»: con dos
                      líneas de transportes distintos —hoy `51984429504` en Cloud
                      API y `51963139984` en whatsmeow— un tag global diría lo
                      que le toca a una sobre las dos. Ausente en «Todas» y «Las
                      mías», que agrupan líneas que pueden diferir. */}
                  {tagDeTransporte(l.transporte) && (
                    <span
                      className={
                        'ml-1 rounded-[3px] px-1 align-middle text-[9px] font-bold tracking-wide ' +
                        (activa ? 'bg-primary-foreground/25 text-primary-foreground' : 'bg-muted text-muted-foreground')
                      }
                    >
                      {tagDeTransporte(l.transporte)}
                    </span>
