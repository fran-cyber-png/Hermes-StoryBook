 ...
 * sobre una función que devolviera `'para ti'` pasaría igual con el componente
...
describe('el rótulo de la cola', () => {
  ...
  test('con el recorte puesto, el número dice de quién es y por qué', () => {
    ...
    const span = m.contenedor.querySelector('span');
    expect(span?.textContent).toBe('2379 para ti');
    ...
    // segunda mitad, «para ti» promete una cola de asignadas y la vendedora
  ...
  test('🔴 con línea propia, la explicación cambia y NO promete lo huérfano', () => {
    ...
    const span = m.contenedor.querySelector('span');
    expect(span?.textContent).toBe('51 para ti');
    const ayuda = span?.getAttribute('title') ?? '';
