...
/**
 * EL MINICALENDARIO DE LA IZQUIERDA — el mapa chico para saltar de día.
 *
 * Dibuja DOS marcas distintas y no una sola: **hoy** (navy con el filete dorado,
 * porque el oro en esta agenda significa tiempo) y **el foco**, el día en el que
 * la vendedora está parada (el anillo azul, el color de lo que se accionó). Son
 * dos preguntas distintas —«¿qué día es?» y «¿qué día estoy mirando?»— y con una
 * sola marca la segunda no tenía respuesta en pantalla.
 */
    ...
    <div className="w-72 rounded-lg border border-border bg-card p-4 shadow-sm">
      ...
      <div className="grid grid-cols-7 gap-0 border border-border/40 rounded">
        {dias.map((fecha) => {
          ...
          // El día en el que la agenda está PARADA. Sin esto el minicalendario
          // pintaba siempre hoy y nada más: se tocaba el 10, el título cambiaba
          // solo, y la grilla seguía señalando el 20 como si el clic no hubiera
          // pasado. Hoy manda cuando son el mismo día — que es el caso normal.
          const esFoco = !esHoy && mismaFecha(fecha, foco);
          ...
          return (
            <div
              ...
              aria-current={esHoy ? 'date' : esFoco ? 'true' : undefined}
              className={`
                ...
                ${esHoy
                  ...
                  : esFoco
                    ? 'bg-secondary text-secondary-foreground ring-inset ring-2 ring-primary'
                    : esDelMes
