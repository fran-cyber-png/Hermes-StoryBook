 ...
 *
 * ⚠️ **Por eso el rótulo del botón CAMBIA con el foco.** Pegado al título, un
 * «Hoy» fijo deja de ser un botón y se lee como el rótulo de la fecha que tiene
 * al lado: parada en otro día, la barra decía «Hoy · lunes, 10 de agosto» y
 * juraba que el 10 era hoy —el mismo día en que era 20—. Solo cuando el foco
 * está en hoy el botón dice «Hoy» (y se enciende, porque describe dónde estás);
 * cuando no, dice **«Volver a hoy»**, que es lo que hace. El estilo apagado no
 * alcanzaba: de un vistazo se lee el texto, no el borde.
 */
...
interface CalendarHeaderProps {
  ...
  overdue?: number;
  /**
   * `true` cuando el foco ya está en hoy —en mes, semana y Gantt, cuando hoy CAE
   * en la ventana que se mira—: el botón dice «Hoy», se enciende y no mueve nada.
   */
  enHoy?: boolean;
  onToday: () => void;
  onPrevious: () => void;
    ...
    <div className="flex flex-col gap-3 border-b border-border bg-card p-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="min-w-0 flex-1 sm:px-4">
        <div className="flex flex-wrap items-center justify-center gap-2">
          ...
          >
            {enHoy ? 'Hoy' : 'Volver a hoy'}
          </button>
          {/* `h2` y 18 px: el período es el encabezado de ESTA vista, no el título de
              la pantalla — ése es el `TituloDeSeccion` de la barra de arriba, que
              ahora mide 24. Empatarlo dejaba dos títulos del mismo peso. */}
