 ...
 * pill de la semana — y nada se envía solo: la agenda te avisa, tú haces.
 *
 ...
 *
 * En la COLUMNA IZQUIERDA, debajo del minicalendario, vive **el resumen de
 * próximas actividades** (`ProximasActividades`), que contesta «¿qué sigue?»
 * sin obligar a recorrer la grilla, y cada renglón abre este mismo detalle.
 * Estaba al pie del calendario, donde le disputaba el alto a la grilla y la
 * columna quedaba con un hueco vacío del alto de la pantalla.
 */
          ...
          <div className="mt-4">
            ...
            <SelectorImportancia
              ...
              onChange={(importancia) => {
                actualizarImportancia.mutate({ id: r.id, importancia }, { onError: () => setError('No se guardó la importancia — prueba de nuevo.') });
              }}
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button
              ...
              onClick={() => {
                cambiarEstado.mutate(
                  ...
                  { onSuccess: onCerrar, onError: () => setError('No se guardó el cambio — prueba de nuevo.') },
                );
              }}
              className={
            ...
            {confirmaBorrar ? (
              <button
                type="button"
                onClick={() => {
                  borrar.mutate(r.id, { onSuccess: onCerrar, onError: () => setError('No se borró — prueba de nuevo.') });
                }}
...
function Crear({
  ...
  async function guardar() {
    ...
    try {
      ...
      setErrorGuardar('No se guardó el seguimiento — prueba de nuevo.');
    }
  }
        ...
        >
          ...
          <div className="min-h-0 flex-1 divide-y divide-border overflow-y-auto border-t border-border">
            ...
            <FilaCampo icono={<Phone size={17} />}>
              ...
              {sinAtar && (
                <p className="mt-1 px-0.5 text-[11px] text-destructive">
                  ...
                  {confirmarSinAtar && ' Toca Agendar de nuevo para confirmar.'}
                </p>
              )}
            </FilaCampo>
            ...
            <FilaCampo icono={<Flag size={17} />}>
              <p className="py-1.5 text-[11px] text-muted-foreground">Nada se envía solo: la agenda te avisa, tú haces.</p>
            </FilaCampo>
          </div>
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-background">
      ...
      <div className="flex min-h-0 flex-1 gap-4 overflow-hidden p-4">
        {/* ── COLUMNA IZQUIERDA: el minicalendario y, DEBAJO, lo que sigue ──
            El resumen vivía como franja al pie del calendario central, donde
            competía por el alto con la grilla. Acá ocupa el hueco que la
            columna ya tenía vacío y queda a la misma altura que los días, que
            es donde se lo mira: «¿qué hay el 18?» arriba, «¿qué sigue?» al
            lado. El calendario central NO se toca — sigue siendo el mismo
            bloque, con el mismo alto disponible. */}
