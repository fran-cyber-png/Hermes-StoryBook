...
export function useAgenda() {
  ...
  const agenda = useQuery({
    ...
    // "mañana" se vuelve "vencido" aunque nadie toque nada, y eso es el reloj
    // del CLIENTE (`VistaAgenda` ya lo recalcula cada 60 s por su cuenta): lo
    // único que este poll trae de nuevo es una mutación AJENA (otra pestaña
    // mía creando un recordatorio), y para eso 5 minutos con el stream vivo
    // alcanza (`docs/plan-borrar-el-polling.md` §6 PR 1).
 ...
 * (`VistaAgenda` lo usa de fecha inicial del formulario). Agrega al final o al
