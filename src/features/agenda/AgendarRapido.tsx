...
export function AgendarRapido({
  ...
  // que la vista Agenda y el riel: dos fuentes para «qué hay ese día» sería #37.
  const porDia = agruparPorDia(agenda.data?.recordatorios ?? []);
        ...
        <>
          ...
          <div className="absolute right-0 top-8 z-30 w-72 rounded-xl bg-card p-2.5 shadow-panel">
            ...
            <div className="mt-2 flex items-center gap-1.5 border-t border-border pt-2">
              <SelectorFechaHora valor={fechaElegida} onSeleccionar={setFechaElegida} porDia={porDia} />
              <button
                type="button"
                disabled={!fechaElegida || pendiente != null}
                onClick={() => {
                  if (!fechaElegida) return;
                  void agendar({
                    etiqueta: fechaElegida.toLocaleString('es', {
                      day: 'numeric',
                      month: 'short',
                      hour: '2-digit',
                      minute: '2-digit',
            ...
            {crear.isError && <p className="mt-1.5 text-[11px] text-destructive">No se agendó — prueba de nuevo.</p>}
