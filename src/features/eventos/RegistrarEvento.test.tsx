...
function abrir(m: Montado) {
  const disparador = m.contenedor.querySelector<HTMLButtonElement>(
    'button[title="Anotar algo en el timeline del contacto (N)"]',
  );
...
/**
 * El catálogo de tipos vive en un flyout aparte desde el 27-ago-2026 (pedido
 * del dueño: el input libre es lo primero que se ve). Un chip del catálogo ya
 * no está a un click del disparador — está a dos.
 */
function abrirNotasRapidas(m: Montado) {
  const boton = [...m.contenedor.querySelectorAll('button')].find(
    (x) => x.textContent?.trim() === 'Notas rápidas',
  );
  if (!boton) throw new Error('no encontré el botón «Notas rápidas»');
  boton.click();
}
 ...
 *
 * Ojo: el disparador dice «Registrar algo del contacto» y viene antes en el
 * DOM, así que un `find` por texto sobre todo el contenedor puede devolver el
 * disparador —que nunca está deshabilitado— y el test pasa mirando el botón
 * equivocado. Por eso se busca dentro de `.shadow-panel`.
 */
...
describe('el popover de registrar', () => {
  ...
  test('el botón nace deshabilitado y el motivo se DICE, no se adivina', async () => {
    ...
    expect(montado.contenedor.textContent).toContain('Escribe qué pasó');
  });

  test('🔴 `senalAbrir` abre el popover — es lo que el atajo `N` dispara desde el shell', async () => {
    montado = montar(<RegistrarEvento clave={CLAVE} senalAbrir={0} />);
    await reposar();
    expect(estaAbierto(montado)).toBe(false);

    montado.repintar(<RegistrarEvento clave={CLAVE} senalAbrir={1} />);
    await reposar();
    expect(estaAbierto(montado)).toBe(true);
  });
