 ...
 *
 * ⚠️ **`data-esqueleto` no es decorativo: es lo que mira el candado.**
 * `PanelDerecho.campana.test.tsx` fija que este skeleton —el del lead-form— no
 * aparece en campaña, y lo hacía buscando `.animate-pulse` en todo el panel. Esa
 * clase dejó de identificarlo el día que el panel tuvo un segundo esqueleto
 * legítimo: el test se ponía rojo por el motivo equivocado, y aflojarlo a «que
 * haya menos de N» habría apagado el candado entero.
 */
export function BloqueMetaSkeleton({ cantidad }: { cantidad: number }) {
  return (
    <div
      className="grid grid-cols-2 gap-x-4 gap-y-3 max-[340px]:grid-cols-1"
      aria-hidden
      data-esqueleto="meta"
    >
      {[0, 1].map((i) => (
