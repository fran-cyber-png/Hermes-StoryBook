...
export function estadoDelContacto(e: EntradaEstadoContacto): EstadoContacto {
  ...
  /**
   * 🔴 EN CAMPAÑA NO SE DICE NADA — NI SIQUIERA «SIN FICHA».
   *
   * Decía «Sin ficha · La ficha de Cerberus es del módulo de ventas», y era
   * correcto y ruido a la vez: le explicaba a un operador de campaña que existe
   * un módulo de ventas cuyo ERP nunca va a ver. Un badge que nombra el sistema
   * de OTRO negocio no es información, es la frontera contada al revés — y la
   * pantalla de campaña tiene que leerse como si la Escuela no existiera.
   *
   * `titulo: ''` es lo que apaga el badge (`EncabezadoTimeline` no dibuja uno
   * vacío), y no un flag nuevo: el estado ya viaja entero, y una bandera aparte
   * sería una segunda cosa que hay que acordarse de mirar.
   *
   * ⚠️ Sigue devolviendo `tono: 'otro-canal'`: quien pinta la banda necesita un
   * tono válido igual, y ese es el neutro que ya existía.
   */
  if (e.sinCerberus) {
    return { ...base, tono: 'otro-canal', acento: 'neutro', titulo: '', detalle: null };
  }
    ...
    if (e.padron) {
      return {
        ...
        detalle:
          ...
          'Verifica antes de tratarlo como nuevo.',
      };
    }
    return {
      ...base,
