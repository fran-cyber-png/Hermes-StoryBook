...
describe('FichaRapida', () => {
  ...
  /**
   * 🔴 **«Empresa» es de ventas, y las DOS mitades importan.** No dibujarlo es
   * la mitad fácil; la que muerde es que `prellenar` lo llena SOLO partiendo el
   * alias de WhatsApp, así que sin vaciarlo al mandar se guardaría un dato que
   * la persona nunca vio y no puede corregir. Un campo invisible que igual se
   * escribe es peor que uno de más: aparece semanas después, en la ficha de otro.
   *
   * El caso usa el MISMO alias que el de ventas («Jorge Martin - JM RUSH
   * AUTOMOTRIZ»), que es el que sí tiene empresa: con un alias sin guion esto
   * pasaría en verde aunque el vaciado no existiera.
   */
  test('🔴 en campaña «Empresa» no se dibuja NI se guarda', async () => {
    montado = montar(<FichaRapida conversacion={CONVERSACION} onCerrar={() => {}} esDeCampana />);
    await reposar();

    expect(
      montado.contenedor.querySelector('input[aria-label="Empresa"]'),
      'el campo Empresa no va en campaña: dónde trabaja alguien no cambia nada de lo que se hace con él',
    ).toBeNull();
    ...
    const post = pedidos.find((p) => p.body != null);
    expect(post?.body).toMatchObject({ nombre: 'Jorge', apellido: 'Martin' });
    expect(
      (post?.body as { empresa?: string })?.empresa,
      'el prellenado sacó «JM RUSH AUTOMOTRIZ» del alias y se mandó igual, sin que nadie pudiera verlo',
    ).toBe('');
  });

  test('LA OTRA MITAD: en ventas «Empresa» sigue estando y sigue viajando', async () => {
    montado = montar(<FichaRapida conversacion={CONVERSACION} onCerrar={() => {}} />);
    await reposar();
    expect(campo(montado, 'Empresa').value).toBe('JM RUSH AUTOMOTRIZ');
  });
