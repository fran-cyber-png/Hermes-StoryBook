...
function motivoDelError(e: unknown): string {
  ...
  if (e instanceof ErrorApi && e.status === 404) return 'Ese dato ya no existe: refresca la pantalla';
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-8">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Tienes <strong className="text-foreground">{activos}</strong> datos prendidos. En cada momento de
      ...
      <p className="mt-4 text-[11px] leading-snug text-muted-foreground">
        ¿Falta uno que cargaste? Míralo en la lista: si dice <strong>«no se ve»</strong>, hay otros
        con <strong>orden</strong> más bajo que le ganan el lugar. Bájale el número.
      </p>
    </div>
  );
}
      ...
      : rotulo.trim().length > 30
        ...
        : texto.trim().length < 5
          ...
          : texto.trim().length > 600
            ...
            : clave.length < 2
              ? 'Del rótulo no sale una clave usable: pon al menos dos letras o números'
