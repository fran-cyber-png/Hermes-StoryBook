 ...
 * Cerberus dice **«abre la conversación»** — que desde el panel derecho es un
  ...
  return (
    <div className="space-y-3 rounded-xl border border-border bg-muted/30 p-3">
      ...
      {formError && (
        <div className="flex items-start gap-2 rounded-lg border border-warning/40 bg-warning/10 p-2.5 text-xs text-warning-foreground">
          ...
          {formErrorObj instanceof ErrorApi
            ...
            : 'No se pudo cargar el formulario de Cerberus. Prueba salir de Hermes y volver a entrar.'}
        </div>
      )}
      ...
      <CampoAlta label="País">
        ...
        >
          <option value="">{cargandoForm ? 'Cargando…' : 'Elige un país'}</option>
          {form?.paises.map((p) => (
        ...
        {form?.paisIdSugerido && paisId === form.paisIdSugerido && (
          <span className="text-[11px] text-muted-foreground">
            adivinado por el prefijo — cámbialo si hace falta
          </span>
        )}
      </CampoAlta>
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <CampoAlta label="Prefijo">
            <select
              ...
              className={fieldClass}
            >
              <option value="">{cargandoForm ? 'Cargando…' : 'Elige un prefijo'}</option>
        ...
        {form?.telefonoSugerido && !form.telefonoSugerido.prefijo && !telefonoPrefijo ? (
          ...
          <span className="text-[11px] text-muted-foreground">
            el +1 es de varios países (EE. UU., Canadá, Rep. Dominicana) — elige cuál es
          </span>
        ) : (
          ...
          telefonoNumero === form.telefonoSugerido.numero && (
            <span className="text-[11px] text-muted-foreground">
              adivinado del número de esta conversación — cámbialo si hace falta
            </span>
          )
        )}
      </div>
...
export function VentaDesdeElPanel({
  ...
  /** La moneda que ya eligió en el mismo carrito — el id de Cerberus, no el código. */
  monedaInicial?: string;
  /** Avisa cuando la moneda cambia ACÁ, para que el carrito la refleje también. */
  ...
  if (data?.estado === 'cliente') {
    return (
      <FormularioVenta
        ...
        onMonedaCambiar={onMonedaCambiar}
      />
    );
  }
        ...
        {isPending ? (
          ...
          <div className="flex items-start gap-2 rounded-xl border border-warning/40 bg-warning/10 p-3 text-xs text-warning-foreground">
            ...
            <span>
              ...
              cliente: es que la ficha no cargó. Prueba de nuevo en un rato.
            </span>
          </div>
        ) : !telefono ? (
