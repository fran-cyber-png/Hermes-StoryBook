...
/**
 * UN PRODUCTO ELEGIDO — lo que la vendedora arma en el «Carrito» del panel de
 * la ficha (`panel/CarritoDeseado.tsx`), antes de llegar al formulario de
 * venta.
 *
 * `precio` es OPCIONAL y a propósito (decisión del 20-ago-2026, revierte la
 * de 19-ago): «sin precio acá» dejaba a la vendedora sin poder anotar cuánto
 * habían negociado mientras charlaba, y al tocar «Registrar venta» el
 * formulario igual arrancaba en el de promoción del catálogo — lo tipeado en
 * el carrito se perdía. Ahora viaja como texto (se está tipeando, como en
 * `FormularioVenta.Linea`) y `FormularioVenta` lo usa como arranque de esa
 * línea; sigue siendo editable ahí, porque el precio final se sigue
 * negociando recién al cerrar.
 */
...
/** Lo tipeado en un precio, como número usable. Nunca negativo: un descuento no es una devolución. */
export function precioDe(texto: string | undefined): number {
  if (!texto) return 0;
...
/**
 * El total del carrito, agrupado por MONEDA — la misma regla que
 * `resumenCompras` en `cerberus/ficha.ts`: sumar S/ con USD daría un número
 * que no existe en ninguna parte. Una línea sin precio (todavía no se
 * negoció) no suma ni figura: no es un descuento a cero, es que no se sabe.
 *
 * `monedaPorDefecto` es la que el carrito muestra cuando el catálogo de
 * Cerberus no trae la suya (`ProductoCurso.moneda === ''`, el caso de HOY
 * para casi todo el catálogo) — la elige la vendedora en el propio carrito,
 * nunca se adivina.
 */
export function totalCarrito(
  ...
  for (const l of productos) {
    const monto = precioDe(l.precio) * l.cantidad;
    if (monto <= 0) continue;
    const moneda = l.producto.moneda || monedaPorDefecto;
    porMoneda.set(moneda, (porMoneda.get(moneda) ?? 0) + monto);
  }
  return [...porMoneda.entries()].map(([moneda, total]) => ({ moneda, total }));
}
...
export function useCrearVenta() {
  ...
  return useMutation({
    mutationFn: (v: {
      ...
      api<{ ok: true; folio?: string; mensaje?: string; ventaId?: number }>('/api/venta/crear', {
        method: 'POST',
...
/**
 * MANDAR EL PDF DE LA COTIZACIÓN — el mismo que Cerberus genera, como adjunto
 * de WhatsApp. Un clic, un envío (mismo criterio que `DosRespuestas`): no pasa
 * por el composer porque no hay texto que editar, es un archivo.
 */
export function useEnviarCotizacion() {
  return useMutation({
    mutationFn: (v: { ventaId: number; telefono: string; numeroPropio: string; referencia: string }) =>
      api<{ ok: true; idExterno?: string }>('/api/venta/cotizacion/enviar', {
        method: 'POST',
        body: JSON.stringify(v),
      }),
  });
